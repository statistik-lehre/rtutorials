...
Nun wechseln wir zu anderen Beispieldaten, weil sich Quantile schlecht erklären lassen anhand von lediglich 5 Datenpunkten. 
Nehmen wir noch mal unser Eingangsbeispiel, was einfach zufällig aus einer Normalverteilung gezogene Daten waren - nur dieses mal mit etwas weniger Datenpunkten, 100 an der Zahl.

### Proof of Concept

Es folgt eine Erklärung in Bildern und Schritten, die das Konzept vermittelt, ohne auf technische Details einzugehen.

```{r quantileintro}
 set.seed(1)
       data <- rnorm(100)
       df2 <- data.frame(count = 1:100, data, sorted = sort(data))
       ggplot(df2, aes(count, data)) +
         geom_point() +
         labs(y = "x", x = "Index") +
         theme_bw()
```

Zunächst müssen wir die Daten ihrer Größe nach sortieren:

```{r quantilesort}
 ggplot(df2, aes(count, sorted)) +
         geom_point() +
         labs(y = "x", x = "Index") +
         theme_bw()
```
...
Das 25% - Quantil ist der Wert, unterhalb dem 25% der Werte liegen. 

```{r quartile1}
 ggplot(df2, aes(count, sorted)) +
         geom_point() +
         geom_hline(yintercept = df2$sorted[26]) +
         geom_label(y = df2$sorted[26], x = 0, label = "Q1") +
         labs(y = "x", x = "Index") +
         theme_bw()
```
...
```{r}
 ggplot(df2, aes(count, sorted)) +
         geom_point() +
         geom_hline(yintercept = quantile(df2$sorted, probs = c(0.25, 0.5, 0.75))) +
         annotate(geom = "label", 
                  y = quantile(df2$sorted, probs = c(0.25, 0.5, 0.75)), 
                  x = 0, label = c("Q1", "Q2", "Q3")) +
         labs(y = "x", x = "Index") +
         theme_bw()
```
...
Es ist klar zu sehen, dass die mittleren 50 % auf viel dichterer Fläche konzentriert sind als die 25% darüber und darunter. 
...
```{r, include = FALSE}
# Hier hab ich getestet, ob es mit vertauschten Achsen besser aussieht
 ggplot(df2, aes(sorted, count)) +
         geom_point() +
         geom_boxplot(alpha = 0.8, y = 50, ymin = 0, ymax = 100, fill = "pink") +
         annotate(geom = "label", 
                  x = quantile(df2$sorted, probs = c(0.25, 0.5, 0.75)), 
                  y = 0, label = c("Q1", "Q2", "Q3")) +
         labs(x = "x", y = "Index") +
         theme_bw()
```
...
Es wurde nur erwähnt, dass ein $n \%$-Quantil der Wert ist, unter dem $n \%$ der Werte liegen. Aber wie genau wird dieser Wert bestimmt?
...
Die Punkte aus den obigen Grafiken sind im Vektor `x` gespeichert. Verwenden Sie die `quantile()`-Funktion, um sich die Quartile ausgeben zu lassen.
