...
Das soll hier aber nur ein kleiner Einblick in das sein, was alles möglich ist. In diesem Tutorial starten natürlich bei den Basics und fokussieren uns darauf grundständige Diagramme zu zeichnen. Mit diesem Grundwissen kannst du dir dann auch selbst beibringen solche abgefahrenen Diagramme zu erstellen.
...
|--------------------|:--------------------------|:---------------------------------|
| **keine**          | Balekdiagramm <br>`geom_bar()`| Histogramm <br>`geom_histogram()`|
...
:::
<br>
*Beachte, dass hier die Argumentnamen `data` und `mapping` weggelassen wurden, so wie es in der Praxis üblicherweise auch getan wird. Das ist möglich, da die Reihenfolge der Argumente beachten wire, also erst `data`, dann `mapping` und schließlich `geometric objects`.* 
...
Es gibt hier einen Unterschied, in den R Skripten, mit denen du arbeitest und den R Tutorials, in denen du dich gerade befindest, deswegen die folgende Aufgabe:
...
quiz(caption = "Quiz zu den Balkendiagrammen",
     question("Welche Daten lassen sich gut mit Balkendiagrammen darstellen?",
               ...
               answer("Eine einzelne, stetig Variable.",
                      ...
                      Es ist eine einzelne Variable, aber sie ist nicht stetig, versuch es nochmal. 
                      Für diese Datenart eignet sich ein Histogramm."),
               answer("Zwei kategoriale Variablen.",
                      message = "Schon fast richtig. 
                      Die Variable ist kategorial, aber es sind nicht zwei.
                      Für diese Datenart eignet sich eine Kreuztabelle."),
...
question("Welche drei Kernbestandteile hat laut der Grammar of Graphs Theorie von Leland Wilkinson 
         ...
         question("Welches sind keine ästhetischen Attribute?",
               ...
               answer("Statistische Kennzahlen",
                      correct = T),
               answer("Wenn die Farben schön zusammenpassen.",
                      message = "Das ist zwar ästhetisch, aber kein ästhetisches Attribut nach der GG Theorie. 
                      Farbe per sé ist so eines, aber wie schön die sind oder wie gut sie zusammenpassen 
                      ist keines."),
               allow_retry = TRUE,
               random_answer_order = TRUE,
               correct = "Richtig!",
...
::: infobox
Ein Boxplot ist eine effektive und robuste Methode, um die Verteilung **einer stetigen** und **einer kategorialen** Variable zu visualisieren und Ausreißer zu identifizieren.
...
quiz(caption = "Interpretation üben",
     question("Welche Daten lassen sich gut im Boxplot darstellen?",
               answer("Eine einzelne, kategoriale Variable.",
                      message = "Fast richtig. Nur dass noch eine weitere, stetige Variable benötigt 
                      Hier würde sich ein Balkendiagramm anbieten."),
               answer("Eine einzelne, stetig Variable.",
                      message = "Fast richtig. Es fehlt noch eine weitere, kategoriale Variable. In diesem Fall eignet sich ein Histogramm."),
               answer("Eine kategoriale und eine stetige Variable,",
                      correct = T),
               answer("Zwei stetige Variablen.",
                      message = "Fast richtig. Nur das die zweite Variable kategorial ist und nicht stetig. 
                      Für diese Datenart eignet sich ein Punktdiagramm."),
               allow_retry = TRUE,
               random_answer_order = TRUE,
               correct = "Richtig!",
...
ggplot(einkaufen, aes(x = befragung, y = x)) +
  geom_count()
```
