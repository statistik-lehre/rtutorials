...
Als letztes Wort zu den Ästhetischen Attributen, und das wohl am beeindruckenste: Das Vereinen von `aes()` und `geom()` mit der Funktion wie `scale_fill_manual()`. 
...
## 7. Das Drumherum: Themes, Labs und Legenden
...
Wie bekommst du diese Farbpaletten in deine Plots? Du nutzt wie gewohnt den `+` Pipe Operator um eine weitere Funktion in `ggplot()` zu füttern. Du kennst bereits die manuelle Variante `scale_fill_manual()`, mit der du den Kategorien des `aes()` `color`-Attributs mittels dieser Funktion manuell selbstgewählte Farben zugeteilt hast. Farbpaletten nehmen dir diese Arbeit ab, solange du darauf achtest die entsprechende Funktion anhand des Skalenniveaus deiner farbgebenden Variable zu wählen. Hier eine kurze Tabelle mit einer kleinen Auswahl an möglichen Funktionen aus dem Paket `viridis`:
...
::: aufgabe
Du kannst es dir bestimmt schon denken: Wähle aus der Tabelle die geeignete Funktion und aus und ersetze damit die dir bekannte Funktion `scale_fill_manual(values = c("#1f661b", "purple"))`. 
:::
...
::: aufgabe
Suche die benötigte Funktion anhand der Tabelle raus und nutze eine `viridis` Farbpalette deiner Wahl.

(`"viridis"` (default), `"magma"`, `"plasma"`, `"inferno"`, `"cividis"`, `"mako"`, `"rocket"` oder `"turbo"`)
:::
