...
Es gibt (wie immer) tausend verschiedene Wege, Visualisierungen mit R zu erstellen. Aber einer hat sich über die Jahre als besonders beliebt erwiesen, und das ist das 
Paket `ggplot2`. 

Der Grund, warum wir dir diesen Weg beibringen:

- `ggplot2` folgt einem klaren Baukastenprinzip, mit dem auch später beliebig komplexe Anpassungen der Grafik an die eigenen Vorstellungen relativ schmerzfrei umgesetzt werden können. 
- `ggplot2` ist der populärste Weg, daher gibt es auf Google viele gute Anleitungen und Antworten. Und es existieren zahlreiche Eweiterungen um das Paket `ggplot2`. 
...
Mit data kennst du dich ja jetzt schon aus, daher können wir direkt zum *mapping* übergehen. Wichtig ist nur zu wissen, dass `ggplot()` nur *data frames* als Daten
akzeptiert. 
...
::: blau-nb
Beachte, dass hier die Argumentnamen `data` und `mapping` weggelassen wurden, so wie es in der Praxis üblicherweise auch getan wird. Das ist möglich, da die Reihenfolge der Argumente beachten wire, also erst `data`, dann `mapping` und schließlich `geom`etric objects.
:::
...
Innerhalb der Funktion für das geometrische Objekt finden auch Berechnungen statt, die einzelnen Ausprägungen der Verkehrsmittel werden gezählt, um die Höhe der Balken zu bestimmen. Aber das übernimmt zum Glück alles R. 
...
- Schon die erste Zeile gibt dir eine wichtige Information: Der Fehler trat beim
Hinzufügen des `geom`etrischen Objekts auf und nicht an irgendeiner anderen Stelle
- Die zweite Zeile sagt, dass der Fehler in der ersten Zeile / Ebene passiert ist. 
Das wird relevant, wenn du mehrere Ebenen mit `+` übereinanderlegst in deinem Plot.
- Die dritte Zeile musst du nicht genau verstehen, das ist irgendwas internes von
`ggplot2`
...
question("Welche Aussagen über das obige Balkendiagramm mit den Vekehrsmitteln treffen zu?",
               answer("Das Auto war das Häufigsten genutzte Fahrzeug zum Einkaufen.",
                      correct = TRUE),
               answer("Es nutzten ca. 38 Menschen das Fahrrad zum einkaufen.",
                      correct = TRUE),
               answer("Ca 28% der Menschen nutzten ÖPNV zum einkaufen",
                      message = "Fast richtig. 
                      Allerdings wird auf der y-Achse die absolute Anzahl der Beobachtungen gezeigt, 
                      nicht die prozentuale."),
               answer("Ca. 41% der Befragten finden, dass es am besten ist, zu Fuß einkaufen zu gehen.",
                      message = "Die Frage war, mit welchem Verkehrsmittel die Befragten gerade da waren, 
                      nicht, welches ihrer Meinung nach das beste ist und auf der y Achse sind absolute Zahlen,
                      keine Prozente!"),
               allow_retry = TRUE,
               random_answer_order = TRUE,
               correct = "Richtig!",
...
question("Welche drei Kernbestandteile hat laut der Grammar of Graphs Theorie von Leland Wilkinson 
         ...
         question("Welches sind keine ästhetischen Attribute?",
               answer("y Achse",
                      message = "Die y Achse ist eines der zentralen ästhetischen Attribute."),
               answer("x Achse",
                      message = "Die x Achse ist eines der zentralen ästhetischen Attribute."),
               answer("Geometrische Objekte",
                      message = "Geometrische Objekte sind ein 
                      gleichwertiger notwendiger Bestandteil zu ästhetischen Attributen, 
                      welche die ästehtischen Attribute abbilden.",
                      correct = T),
               answer("Spalten in einem Datesatz",
                      message = "Das sind Daten, kein visuellen Eigenschaften."),
               allow_retry = TRUE,
               random_answer_order = TRUE,
               correct = "Richtig!",
               incorrect = "Das stimmt leider nicht, probier es noch mal!"),

question_text("Wie erklärst du dir die weiten Anreisewege (bis 600km!) beim Verkehrsmittel „ÖPNV”? 
              (Tipp: Der Bioladen ist in der Nähe eines ICE-Bahnhofs)",
              answer_fn(\(x) correct(messages = "Vermutlich waren einige Menschen zuvor mit dem ICE gefahren, 
                                     und hatten vielleicht auch nur einen Umstieg mit Wartezeit und 
                                     Hunger auf ein Bio-Brötchen oder so. Während der Rest der Menschen 
                                     ihren regulären Wocheneinnkauf machen und aus der Umgebung kommen, 
                                     maximal ein paar Kilometer entfernt.")),
...
question("Warum verändert sich die Höhe zwischen den beiden ersten Histogrammen beim Alter um die 25 von 17 auf 25??",
               answer("Weil die Kategorienbreite breiter wird und somit mehr Menschen umfasst.",
                      correct = TRUE),
               answer("Weil die Kategorienbreite sich ändert und jetzt mehr Menschen zusammengefasst werden.",
                      correct = TRUE),
               answer("Weil das Alter entsprechend angepasst wurde.",
                      message = "Die 25 Beobachtungen haben nichts mit dem Alter 25 zu tun."),
               answer("Weil die Kategorienbreite enger wird und somit mehr Menschen pro Alter zusammenkommen.",
                      message = "Genau anders herum: wenn die Kategorien werden breiter werden, werden mehr Menschen in einer Kategorie zusammengefasst."),
               allow_retry = TRUE,
               random_answer_order = TRUE,
               correct = "Richtig!",
               incorrect = "Das stimmt leider nicht, probier es noch mal!"),

question("Was stimmt bzgl. der Argumente colour und fill?",
               ...
               answer("R ist case sensitive, deswegen muss das Argument colour stehts mit ou geschrieben werden.",
                      message = "Das ist teilweise richtig. R ist case sensitive. In diesem speziellen Fall sind allerdings zwei Schreibweisen des gleichen Arguments möglich."),
               answer("colour bestimmt die Farbe der Umrandungen",
                      correct = T),
               answer("fill bestimmt die Farbe des Hintergrundes",
                      message = "fill bestimmt die Farbe der Füllung des jeweiligen geometrischen Objekts."),
               allow_retry = TRUE,
               random_answer_order = TRUE,
               correct = "Richtig!",
               incorrect = "Das stimmt leider nicht, probier es noch mal!"),

question("Was stimmt bzgl. des Arguments boundary?",
               answer("Spezifiziert die Grenze zwischen zwei bins.",
                      correct = TRUE),
               answer("Spezifiziert die Grenzen des gesamten Plots.",
                      message = "Dafür gibt es andere Funktionen (zB. coord_cartesian())."),
               answer("Spezifiziert die Grenzen, welcher Teil des Plots angezeigt wird.",
                      message = "Dafür gibt es bspw. coord_cartesian()"),
...
question("Letzter Plot: Wie alt ist die älteste Person?",
               answer("Zwischen 85 und 90 Jahren.",
                      correct = TRUE),
               answer("89 Jahre, 11 Monate und 3 Tage.",
                      message = "Das könnte zwar sein, könnte aber auch nicht sein. Du kannst du wissen, das diese Person sich im Intervall zwischen 85 und 90 Jahren befindet."),
               answer("100",
...
::: rot
Das Zusammenfassen von Kategorien sollte vor der Datenerhebung geplant sein, damit 
nicht nachträglich Kategorien zusammengefasst werden und dadurch die Ergebnisse 
verändert werden können. Wenn es nachträglich geschieht, muss es auf jeden Fall
gut begründet und **dokumentiert** werden, sonst ist das wissenschaftlicher Betrug!
:::
...
::: infobox
**Tipp:**
Wenn du die Beschriftungen einer kategorialen Variable auf der `x`-Achse nicht mehr lesen kannst, weil
sie übereinandergedruckt werden: Pack die Variable einfach auf die
`y`-Achse! Dann werden die Beschriftungen untereinander dargestellt und überlappen
sich nicht. 
:::
...
Schau dir das Diagramm mal an. Einige Extremwerte der Variable 
`weg` definieren die Skala der y-Achse auf 0 - 600 km, wodurch die "normalen" Werte am unteren Rand zu einer
undifferenzierbaren Masse zusammenschrumpfen. Im nächsten Kapitel erfährst du
Möglichkeiten, mit extremen Werten umzugehen, und wie das
Punktdiagramm ohne sie aussehen würde.

```{r scatterquiz}
question_checkbox("Plot 1: Wie alt ist die Person, die 600km Anreiseweg hat ca.?",
...
1. Weiter oben im Tutorial wurde bereits erwähnt: Ein Laden, nämlich der Biomarkt Greger, liegt in der Nähe eines ICE-Bahnhofs. Die Ausreißer könnten also von Fernreisenden kommen. Wenn das stimmt, dann müssten die Ausreißerwerte in diesem Laden erhoben worden sein (`ort`).
...
Die Vermutung wird durch die Daten nicht wirklich gestützt. Bei Greger in der
Willi-Allee zeigt der Boxplot gerade mal einen Wert außerhalb von 
$Q3 + 1.5\cdot IQR$ (gekennzeichnet als Punkt). Manche Studierende haben
leider nicht dokumentiert, an welchem Laden sie Daten gesammelt haben, deswegen
gibt es `NA`s bei der Variable `ort`. `NA` heißt, es ist nicht klar, um welchen und wie viele Läden es sich handelt. 
...
Im Normalfall sollte bereits vor der Datenerhebung festgelegt werden, nach welchen
Kriterien Daten ausgeschlossen werden müssen - damit die Versuchsleiter nicht
nachträglich die Möglichkeit haben, das Ergebnis in eine bestimmte Richtung zu lenken.
:::
...
::: infobox
Liniendiagramme sind geeignet, um Verläufe und Entwicklungen darzustellen.
Dafür braucht es **zwei Variablen**: eine **stetige** Variable, und eine andere 
mit einer **sequentiellen Abfolge** (wie z.B. die Zeit). 
...
Hierfür wird ein anderer Datensatz benötigt, da wir für ein Liniendiagramm eine wiederholte Messung brauchen.
`einkaufen` enthält 160 unabhängige Messungen, keine der Personen wurde ein
zweites Mal befragt. Deswegen ist `einkaufen` nicht geeignet um einen Verlauf
...
Im Datensatz `rtutorials::heated` ist die maximale Tagestemperatur (`maxtemp`) in Grad Fahrenheit sowie das Tagesdatum (`enddate`) gespeichert. Es handelt sich um 
wiederholte Messungen, weil der gleiche Gegenstand (die Temperatur in New York) 
immer wieder (täglich) erhoben wurde.
...
Der Umgang mit Datumsformaten in R würde hier komplett den Rahmen sprengen, 
aber die Lösung ist: \
...
**Zu 1**: Bietet sich an, wenn es die Daten dadurch noch übersichtlicher macht bzw. nichts an Übersichtlichkeit verloren geht. Im Beispiel der Variablen `bioladen`: aktuell noch logical würde sie durchaus übersichtlicher werden, wenn Einträge "bio" und "konventionell" wären. Hier begegnen wir unserem alten Bekannten, dem `factor`, wieder. Es wird eine neue Variable mit dem Namen "ladenart" erstellt. Es wäre allerdings auch denkbar, die alte zu überschreiben. 
  ...
  - **Con:** Eher seltener der Fall, da das Datawrangling (s. entsprechendes Tutorial) ja eigentlich schon vorher abgeschlossen sein sollte und die Daten eigentlich in sehr hoher Qualität vorliegen sollten.
...
data_einkaufen$ladenart <- factor(data_einkaufen$bioladen, 
                                  levels = c(TRUE, FALSE), 
                                  labels = c("bio", "konven."))

ggplot(data_einkaufen, aes(x = verkehrsmittel, fill = ladenart)) +
  geom_bar(position = position_dodge(preserve = "single")) +
  labs(
...
\* Steht nur der Vollständigkeit halber hier, ist keinem der vorgestellten geoms sinnvoll einsetzbar. Wird vor allem für Heatmaps verwendet.
  ...
  - **Zusammenfassung:**
    ...
    - **Con:** Erhöhte Fehlergefahr aufgrund von flaschen Beschriftungen.

```{r d_legends_2, exercise = TRUE, exercise.setup = "d_legends",  exercise.cap = "guide_legend()"}
ggplot(data_einkaufen, aes(x = verkehrsmittel, fill = bioladen)) +
  geom_bar(position = position_dodge(preserve = "single")) +
...
ggplot(data_einkaufen, aes(x = alter, y = weg, colour = kochen)) +
 ...
 ggplot(data_einkaufen, aes(x = verkehrsmittel, y = alter, colour = bioladen)) +
