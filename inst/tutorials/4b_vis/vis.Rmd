...
# Ein Subset aus dem aktuellen Datensatz treffen und als neues Objekt (daten_oa) speichern
daten_oa <- einkaufen[einkaufen$weg < maximalwert, ]

# und NAs entfernen
daten_oa <- daten_oa[!is.na(daten_oa$weg), ]
```
...
```{r scatterplot2}
ggplot(daten_oa, aes(x = alter, y = weg)) +
  geom_point()
```
...
ggplot(daten_oa, aes(x = alter, y = weg, colour = bioladen)) +
  geom_point()
```
...
```{r scatterplot2, exercise.setup = "ausreisser5"}
ggplot(daten_oa, aes(x = alter, y = weg, colour = kochen)) +
  geom_point()
```
...
ggplot(daten_oa, aes(x = alter, y = weg, shape = verkehrsmittel)) +
  geom_point()
```
...
```{r exercise.setup = "ausreisser5"}
ggplot(daten_oa, aes(x = alter, y = weg, shape = verkehrsmittel, colour = verkehrsmittel)) +
  geom_point()
```
...
```{r scatterplot2, exercise.setup = "ausreisser5"}
ggplot(daten_oa, aes(x = alter, y = weg, size = bioladen)) +
  geom_point()
```
...
```{r}
ggplot(daten_oa, aes(x = alter, y = weg, alpha = kochen)) +
  geom_point()
```
...
```{r}
ggplot(daten_oa, aes(x = alter, y = weg, colour = verkehrsmittel, shape = ort, size = bioladen, alpha = kochen)) +
  geom_point() +
...
Für ein oder zwei Variablen. Um das gut zu demonstrieren wird ein größerer Datensatz benötigt: In `fallcolors` wird die tägliche Prozentzahl von gefärbten Blättern im Herbst im Mount Mansfield State Forest. 

```{r}
# hier lieber Lukas' Grafik einsetzen
ggplot(fallcolor, aes(x = as.numeric(day), y = value, colour = year, group = year)) +
  geom_line() +
  facet_grid(elevation ~ species)
```

Vor der Welle (Tilde) stehen die Zeilen und nach der Tilde die Spalten (Reihenfolge der Indizierung bleibt also wie gewohnt). Wenn du nur eines von beiden willst kannst du einfach die Stelle vor bzw. nach der Tilde leer lassen.

### `facet_wrap()`

Für den Fall, dass die Facetten nur anhand einer Variable aufmachen möchtest, du aber flexibler in der Form bleiben möchtest.

```{r}
# hier lieber Lukas' Grafik einsetzen
ggplot(fallcolor, aes(x = as.numeric(day), y = value, colour = year, 
                      group = interaction(year, elevation))) +
  geom_line() +
  facet_wrap( ~ species)
