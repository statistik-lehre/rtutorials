...
ggplot(einkaufen, aes(x = alter, y = weg, color = ort)) +
  geom_point() +
  scale_y_log10()
...
Auch mittels des `shape` Arguments lässt sich die Information einer kategorialen Variable in ein Punktediagramm einbeziehen.
...
Auch Mittels der Größe kann eine weitere Dimension in den Plot gebracht werden. Das ist allerdings mit Vorsicht zu genießen, da es in vielen Fällen die Wahrnehmung zu bestimmten Zusammenhängen verzerrt:

```{r am_size, exercise = TRUE,   exercise.cap = "size"}
ggplot(einkaufen, aes(x = alter, y = verkehrsmittel, size = weg)) +
...
Meist wird `size` als ästhetisches Attribut in Momenten, in denen mit der Größe wirklich eine Relation ausgedrückt werden kann (bspw. Gewicht, Quadratmeter oder Bruttoinlandsprodukt) genutzt.

### Alpha

Zu guter letzt folgt der `alpha` Wert, der die Transparenz von Objekten für das *mapping* nutzbar macht. 

Jedoch ist auch hier Vorsicht geboten, nur weil etwas möglich ist, heißt es nicht, dass es sinnvoll ist. Schau dir doch mal an, was passiert, wenn wir die Verkehrsmittel als Kategorien über `alpha` mappen:
...
Durch die Transparenz wird eine unterschiedliche Gewichtung der Daten suggeriert, die so zwischen den Kategorien natürlich nicht besteht. `alpha` auf diese Art zu nutzen solltest du also vermeiden, da eignen sich Farben und Formen doch deutlich besser. 
...
Jedes geom_---() hat seine eigenen Attribute, um es für deine Zwecke anzupassen. Du findest diese wie immer dort, wo du Hilfe bekommst - in den R Studio Hilfe Seiten oder auch im entsprechenden [Cheat Sheet](https://rstudio.github.io/cheatsheets/data-visualization.pdf) des Paketes. 

Was du aber noch wissen solltest ist, wie du ohne eine Variable die Farbe anpassen kannst. Da die Geom Funktionen einen einzelnen Wert für ihre Attribute benötigen, um sie auf alle Daten anzuwenden, kannst du die Standardfarben auf englisch ausschrieben z.B. "black" oder "green" oder für individualisierte Farben den jeweiligen **Hex-Code** (das sieht dann u.a. so aus "#1f661b", mehr [hier](https://www.color-hex.com/color-wheel/)) verwenden. 
...
Als letztes Wort zu den Ästhetischen Attributen, das wohl am beeindruckenste: Das Vereinen von aes() und geom() mit der Funktion `scale_fill_manual()`. 
...
Es werden die Infomationen der Variable `bioladen` hinzugefügt und eine Legende erstellt, aber du kannst diesen Kategorien selbst Farben zuordnen. Logischerweise benötigt die Funktion dafür entsprechend der Anzahl an Kategorien, die gleiche Anzahl an Farben. 
