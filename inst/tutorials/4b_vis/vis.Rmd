...
```{r linegraph3}
# Für die Abschnitte auf der x-Achse
...
**Zu 1**: Bietet sich an, wenn es die Daten dadurch eh übersichtlicher macht bzw. nichts an Übersichtlichkeit verloren geht. Im Beispiel der Variablen `bioladen` sehr ersichtlich: aktuell noch logical würde durchaus übersichtlicher werden, wenn Einträge "bio" und "konventionell" wären. Hier begegnen wir unserem alten Bekannten, dem `factor` wieder. Es wird eine neue Variable mit dem Namen "ladenart" erstellt. Es wäre allerdings auch denkbar, die alte zu überschreiben. 
...
ggplot(data_einkaufen, aes(x = verkehrsmittel, fill = bioladen)) +
  geom_bar(position = position_dodge(preserve = "single")) +
  scale_fill_discrete(labels = c("bio", "konven."),
...
::: aufgabe
Finde für die Funktionen `scale_colour_continuous()` und `scale_colour_continuos()` eine Anwendungsmöglichkeit in einen der bereits ausgeführten Plots dieses Tutorials und füge diese Ebene hinzu. Füge auch gerne ein Theme deiner Wahl sowie passende Achsenbeschriftungen hinzu.
...
ggplot(data_einkaufen, aes(x = alter, y = weg, colour = kochen)) +
   geom_point() +
   scale_colour_continuous(guide_legend(title = "Kochen/ \n Woche")) +
  labs(
    title = "Weg und Alter",
    x = "Alter",
    y = "Weg (in km)"
 ...
 ggplot(data_einkaufen, aes(x = verkehrsmittel, y = alter, colour = bioladen)) +
   geom_boxplot() +
   scale_colour_discrete(labels = c("bio", "konven."),
    guide_legend(title = "Ladenart")) +
   labs(
...
Optional gibt es noch die Möglichkeit, sich eine Palette (s. [Hilfeseite](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html)) auszusuchen. Das geht mit dem Argument `option = ` und dann eine der Paletten in Anführungszeichen (`"magma"`, `"inferno"`, `"plasma"`, `"viridis"`, `"cividis"`, `"rocket"`, `"mako"` oder `"turbo"`).


::: aufgabe
Finde für jede der vier Kombinationen (colour und fill zu diskret und kontinuierlich) eine Anwendungsmöglichkeit in einen der bereits ausgeführten Plots dieses Tutorials und füge diese Ebene hinzu. Nutze dafür jeweils eine andere Palette. Füge auch gerne ein Theme deiner Wahl sowie Achsenbeschriftung hinzu.
...
ggplot(data_einkaufen, aes(x = verkehrsmittel, fill = bioladen)) +
  ...
  scale_fill_viridis(discrete = T, option = "plasma", 
                     ...
                     labels = c("konventionell", "biologisch")
                     ) +
  labs(
    title = "Verkehrsmittel und Ladenart",
    y = "Absolute Anzahl der Nutzenden",
