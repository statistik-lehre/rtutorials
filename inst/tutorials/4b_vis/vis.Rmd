...
Beachte, dass wir dieses Mal die Argumentnamen `data` und `mapping` weggelassen haben, so wie es in der Praxis üblicherweise auch getan wird. Das war oben nur zur Einführung.
:::
...
Manche Sachen verstehst du vielleicht nicht, wie z.B. was `compute_geom_1()` ist, und die beste Strategie für diese Sachen in R ist sie einfach zu ignorieren und anstattdessen Sinn aus den Dingen zu ziehen, die du verstehst!
...
question_text("Wie erklärst du dir die weiten Anreisewege (bis 600km!) beim Verkehrsmittel „ÖPNV”? (Tipp: Der Bioladen ist in der Nähe eines ICE-Bahnhofs)",
              answer_fn(\(x) correct(messages = "Vermutlich waren einige Menschen zuvor mit dem ICE gefahren, und hatten vielleicht auch nur einen Umstieg mit Wartezeit und Hunger auf ein Bio-Brötchen oder so. Während der Rest der Menschen normalen Einkauf machen und aus der Umgebung kommen, maximal ein paar Kilometer entfernt.")),
...
Da die Balken gerade visuell in einer großen grauen Masse verschwinden, wird noch nicht so ganz deutlich worum es eigentlich geht. Deswegen wäre es hilfreich, wenn alle Balken eine schwarze Umrandung hätten und etwas hellgrauer wären. 
...
und die Höhe der Balken richtet sich danach, wie viele Beobachtungen in diese spezifische 2.5-Jahres-Kategorie fallen. 
