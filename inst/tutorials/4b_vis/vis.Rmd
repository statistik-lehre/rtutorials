...
Und jetzt kommt das praktische: Es gibt noch sehr viele weitere `geom_...()` Funktionen. Zum Beispiel `geom_point()`, welches ein Punktediagramm aus deinen Daten erstellt.
...
Ok, das Diagramm ist jetzt nicht sehr informativ, weil wir nur diese zwei metrischen Variablen in unserem Datensatz zur Verfügung haben, aber du hast gelernt: 
...
Die Funktion `geom_jitter()` zeichnet einzelne Datenpunkte, und fügt diesen einen zufälliges *zittern* hinzu, um sie ein bisschen durcheinander zu schütteln, sodass nicht mehr alle Punkte übereinander liegen und Datenpunkte von anderen überdeckt werden.
...
::: infobox
Der jitter führt in dieser Art der Verwendung nicht zu einem Informationsverlust/-verzerrung, da wir die Punkte horizontal innerhalb der Kategorien variieren lassen und nicht auf der y-Achse mit der metrischen Variable.
:::

<br>

Achte mal auf das Verkehrsmittel "Andere". Wir sehen manchmal wird ein ganzer Boxplot gezeichnet, während lediglich 7 Personen dahinter stehen, und er ist nicht zu unterscheiden von einem Boxplot für "Auto", was 46 Menschen nutzen. Aber mithilfe der Punkte können wir nun sehen, dass dort weit weniger Datenpunkte vorliegen und wir daher bei der Interpretation dieser Gruppe Vorsicht walten lassen sollten, da sie von der Gruppengröße her nicht mit den anderen zu vergleichen ist. Das triff mehr noch für die Kategorien "Motorrad" und "E-Roller" zu.
...
Der Zeitverlauf zeigt uns einen deutlichen "Peak" der Arbeitslosenzahlen um das Jahr 2010. Geschichtsfreunde werden wissen, dass dies wahrscheinlich durch die Wirtschaftskriese aus dem Jahre 2008 bedingt wurde. 
...
Du siehst, Grafiken sind sehr anfällig für unsere Wahl der Darstellung. Bleib kritisch, auch dir selbst gegenüber. 
...
Jetzt kennst du bereits die 5 häufigsten Diagrammformen. Jetzt bringst du Farbe ins Spiel. Du bist einigen *aesthetic attributes* bereits über den Weg gelaufen, denn seien wir ehrlich, ohne sieht es oft wirklich trist und unbrauchbar aus. Hier aber noch einmal von vorne und genau erklärt was sich hinter diesen Argumenten verbirgt:
...
Was allerdings in vielen Fällen sinnvoll ist, ist eine weitere Variable über das Attribut Füllung (`fill`) zu plotten. Das gilt sowohl für *kategoriale* als auch *metrische* bzw. kontinuierliche Daten.
...
question_radio("Welche Variablen können auf das Attribut `fill` gemappt werden?",
...
question_checkbox("aesthetic mapping: Was stimmt?",
...
::: gelb
Die in diesem Kapitel verwendeten Variablen dienen nur zu Darstellungszwecken. Inhaltlich ist kein Zusammenhang o.ä. zu erwarten. 
:::
...
Die Argumente für *factes* sind besonders, da sie durch eine Tilde (`~`) anstelle eines Kommas verbunden werden. Wenn du nur eine Variable zur Aufteilung benutzen möchtest, kannst du  die Stelle vor bzw. nach der Tilde mit einem `.` versehen.
...
question_checkbox("Was stimmt?",
         answer("Facets ermöglichen es auf eine sehr übersichtliche Weise noch weitere Variablen (Informationen) mit in die Grafik zu bringen.",
                 correct = T),
         answer("`facet_grid()` ist besonders für für die Aufteilung anhand von zwei Variablen im Stile einer Kreuztabelle geeignet.",
                 correct = T),
        answer("`facet_wrap()` ist besonders für die Aufteilung anhand einer Variable und in einer selbst gewählten Anordnung geeignet.",
                 correct = T),
        answer("Die Tilde (~) wird in `facet_grid()` für die Trennung der Argumente (der kategorischen Variablen) genutzt",
                 correct = T),
