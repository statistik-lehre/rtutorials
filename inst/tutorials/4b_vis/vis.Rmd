...
### Regressionsmodelle darstellen

Ein Unterpunkt von Punktediagrammen sind Regressionsmodelle grafisch darstellen. In einem entsprechenden Plot kommt klassischerweise noch eine Linie hinzu, welche die Zusammenhänge grafisch darstellt. Also so:

```{r sp_regressionsmodelle, exercise = TRUE, exercise.setup = "ausreisser5",  exercise.cap = "REgressionsmodelle"}
ggplot(data_einkaufen, aes(x = alter, y = weg)) +
  geom_point() +
  geom_smooth(formula = y ~ x, method = "lm", se = F)
```

Die Funktion `geom_smooth(formula = y ~ x, method = "lm", se = F)` 

- rechnet ein Lineares Model (`lm`)
- Mehr zur Modellierung (`y ~ x`) im Tutorial "Regression"
- `se = F` sorgt dafür, dass die Standardfehler nicht mit angezeigt werden.
...
```{r linegraph, exercise = TRUE, "Liniendiagramm"}
...
ggplot(heated, aes(x = enddate, y = weathermetrics::convert_temperature(maxtemp, "f", "c"), 
...
```{r exercise = TRUE, exercise.setup = "d_legends", exercise.cap = "Legenden"}
