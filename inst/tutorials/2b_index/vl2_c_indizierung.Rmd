...
```{r named_vec, exercise=TRUE, exercise.cap = "Vektor mit benannten Elementen erstellen"}
height <- c(Paul = 178,
            Helmut = 182,
            Kathrin = 169,
            Amadou = 184)
...
... Indizieren mit Namen der Elemente

```{r name_index, exercise = TRUE, exercise.setup = "named_vec"}
height["Amadou"]
height[c("Kathrin", "Paul")]
```

## Exkurs: Attribute

```{r attributes, exercise.setup = "named_vec"}
attributes(height)
names(height)
names(height) <- c("Lisa", "Petra", "Aisha", "Maria")
names(height)
height
```
