...
```{r verkehrsmittel}
quiz(
  question(text = "Welches Skalenniveau hat die Variable *Verkehrsmittel*?",
  ...
  question("Was trifft noch auf die Variable *Verkehrsmittel* zu?",
...
Faktor ist eine Objektklasse (s. 2a_vektoren). Um herauszufinden, wie R die entsprechende Variable repräsentiert, kannst du R mit `class(dataframe$vektor)` danach fragen.

Finde heraus, in welcher Objektklasse R die Variable Verkehrsmittel gespeichert hat. Achtung: die Variable wird - ungewöhnlicher Weise - groß geschrieben. (Klassische Quelle für Fehler, da R ja case sensitiven ist.)

```{r nominal-solution}
class(rtutorials::einkaufen$Verkehrsmittel)
```
...
Perfekt, ist schon ein Faktor. Für den Fall, dass es eine andere Klasse gewesen wäre es möglich für temporäre Veränderung `as.factor()` zu verändern oder besser gleich das ganze als Faktor umzucodieren: `data$variable <- factor(data$variable)`. Später noch mehr dazu

Um die Levels eines Faktors herauszufinden kannst du die Funktion `levels()` verwenden. In diesem Fall sieht das Ergebnis so aus:
...
Da auch ordinale Daten kategorial sind, sind auch hier Faktoren empfehlenswert. Du kannst die Werte nämlich mittels des Argumentes `ordered = T` auch sortieren. Faktoren sind auch deshalb spannend, da du zu den Werten auch noch Bezeichnungen speichern kannst.

Die Variable Frequenz ist ordinalskaliert und die ersten 20 Einträge sehen so aus:

```{r frequenz}
head(rtutorials::einkaufen$Frequenz, 20)
```

Die Objektklasse und die Level sind:

```{r ordered_factor}
class(rtutorials::einkaufen$Frequenz)
levels(rtutorials::einkaufen$Frequenz)
```

Wie erwartet ein geordneter Faktor. Das bedeutet, dass R, dem ja sonst Charakterstringt egal sind bzw inhaltlich nicht verarbeiten kann, weiß, dass einmal die Woche weniger ist als alle zwei Wochen. Das ist praktisch. In der Funktion `factor()` gibt es das Argument `ordered = `. Wenn du das auf TRUE setzt, dann speichert R die Levels in einer Reihenfolge. Doch Obacht: R kann ja die Inhalte nicht lesen. Das bedeutet, dass R das nach dem zweitlogischsten Schema sortiert: Anfangsbuchstaben (kommen ja keine Zahlen vor). Deswegen musst du R auch noch sagen, in welcher Reihenfolge die Level gespeichert werden sollen. Das machst du mit dem Argument `levels = c()", und gibst dort die Reihenfolge ein. Um das zu demonstrieren habe ich die Variable Frequenz zurücktransformiert in einen schlichten unsortierten character Vektor mit dem Namen frequenz

```{r of-setup}
frequenz <- as.character(rtutorials::einkaufen$Frequenz)
```


```{r of, exercise = TRUE}
head(frequenz, 20)
class(frequenz)              # es ist definitiv wieder ein charaktervektor
frequenz <- factor(frequenz) # als faktor speichern.
class(frequenz)              # yay, erstmal richtig. Doch dann:
levels(frequenz)             # oh no! das ist ja die falsche Reihenfolge. Deswegen nochmal von vorn:
frequenz <- factor(frequenz, 
                   levels = c("einmal die Woche","mehrfach die Woche", "alle zwei Wochen",  "einmal im Monat", "seltener"), 
                   ordered = T)
...
R ist ziemlich egal, welche Kardnalskala du da hast (also ob sie einen Nullpunkt hat oder nicht). Das ist zwar für dich als Forschende Person ziemlich relevant (was darf ich mit den Zahlen, was nicht) aber für R ist das ziemlich egal. Der entsprechende Datentyp ist `double`. Genau genommen wird die Rationalskala durch `integer` abgebildet. Aber im Grunde kannst du beides auch mit dem jeweils anderem berechnen. Im Datensatz ist das die Variable Weg (in km). Damit kannst du alle Berechnungen durchführen und diese Art der Variable haben wir in den letzten Tutorien schon eingehend erläutert. Das einzige, was da jetzt neu hinzukommt ist für dich das Wissen: "Aha! Bei den numerischen Vektoren handelt es sich also auch um numerische Skalen".
