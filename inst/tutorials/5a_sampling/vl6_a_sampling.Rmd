...
Lasst uns jetzt ebendieses Histogramm noch einmal in R herstellen mit den Daten-Visualisierungsskills die ihr im letzten Tutorial erlangt habt! Wir haben unsere 33 Gruppen im `tactile_prop_red`-Dataframe gespeichert, der im `moderndive`-Paket mitgeliefert wird. 

Führen Sie den Codeblock aus, um sich die ersten 10 von 33 Zeilen anzeigen zu lassen:
...
Im zweiten Schritt berechnen wir den Anteil roter Kugeln mit der `summarize()`-Funktion. Erinnern Sie sich, dass `summarize()` einen Data Frame mit vielen Zeilen als Input nimmt und als Output einen Data Frame mit einer einzigen Zeile zurückgibt, die *summary statistics* wie `mean()` oder `median()` enthält. In diesem Fall nutzen wir die Summe `sum()`:
...
Warum funktioniert das? Weil R `TRUE` wie die Zahl `1` behandelt und `FALSE` wie eine `0`. Also ist das Aufsummieren von `TRUE` und `FALSE` genau wie das Summieren von `1` und `0`. Letztendlich zählt diese Operation die Anzahl aller roten Kugeln. Die Zahl der roten Kugeln schwankt, wenn Sie den Code neu ausführen - Warum? Wegen Stichprobenvariation (*sampling variation*)!
...
Die Proportion lässt sich auch als Prozentzahl interpretieren. Zum Beispiel könnten Sie bei 0.24 sagen, „24% der Kugeln waren rot“. 
...
Erinnern Sie sich, dass im letzten Kapitel 33 Gruppen von Studierenden jeweils die Schaufel genutzt haben, um eine Stichprobe zu ziehen, was zu 33 Stichproben mit jeweils $n = 50$ Kugeln geführt hat. Dann haben wir daraus 33 Proportionen berechnet.
...
::: aufgabe
Nutzen Sie `print(n = ...)` um sich 60 Zeilen anzeigen zu lassen!
:::
...
Beobachten Sie, dass die ersten 50 Zeilen der Spalte `replicate` den Wert 1 tragen, und die nächsten 50 Zeilen den Wert 2, und so weiter, bis zur 33. Das sagt uns dass die ersten 50 Zeilen zur ersten Stichprobe gehören, die nächsten 50 Zeilen zur zweiten, und so weiter. Da sich dieses Muster durchgängig wiederholt, hat `virtual_samples` $33 \cdot 50 = 1650$ Zeilen. 
...
Erinnern Sie sich, dass bei `group_by()` vor `summarize()` die *summary statistics* für jede Gruppe einzeln berechnet werden, unsere Ausgabe also 33 Zeilen haben wird.
...
Sie sehen die ersten 10 Zeilen von 33, was völlig ausreichend ist um zu begreifen, dass wir nun den Anteil roter Kugeln für jede der 33 Stichproben errechnet haben.
...
Beachten Sie, dass wir die Argumente `binwidth = 0.05` und `boundary = 0.4` hinzugefügt haben. Erinnern Sie sich, dass `boundary = 0.4` zu setzen, sicherstellt dass die Grenze einer Kategorie bei 0.4 liegt. Weil `binwidth = 0.05` ebenfalls festgelegt ist, wird das Kategorien mit Grenzen bei 0.30, 0.35, 0.40, 0.45 und so weiter erzeugen. 
...
Beobachten Sie, dass beide Histogramme gewisse Ähnlichkeiten aufweisen, was das Zentrum und die Variation angeht, obwohl sie nicht identisch sind.
...
Beobachten Sie dass `virtual_samples` $1000 \cdot 50 = 50 000$ Zeilen hat.
...
quiz(
  ...
  learnr::question_text(
    text = "Wenn Sie das Histogramm ansehen, würden Sie sagen, es ist wahrscheinlich, eine einzelne Stichprobe mit 30% - 35% roten Kugeln zu ziehen?", 
    placeholder = "Musterlösung erscheint beim Einreichen der Antwort...",
    answer_fn(function(x) correct()),
...
Nehmen wir an, Sie hätten die Wahl zwischen drei verschiedenen Schaufeln, um Stichproben zu ziehen: Mit 25, 50 oder 100 Slots.

![Drei Schaufeln für drei verschiedene Stichprobengrößen](images/three_shovels.png){width=100%}

<br>

Wenn Ihr Ziel immer noch ist, die Proportion der roten Kugeln in der gesamten Schüssel möglichst gut abzuschätzen, welche der Schaufeln würden sie verwenden?
...
Diese Schritte haben wir im Hintergrund automatisiert und ein Interface geschrieben, wo Sie die Inputs hineingeben und das Histogramm hinausbekommen. 

::: aufgabe
Experimentieren Sie mit der Stichprobengröße, während die Zahl von Stichprobenziehungen konstant bei 1000 bleibt.
...
quiz(
  caption = "Learning Check",
  learnr::question_text(
    text = "Welches Muster haben Sie beim Verändern der Stichprobengröße bemerkt?", 
    placeholder = "Musterlösung erscheint beim Einreichen der Antwort...",
    answer_fn(function(x) correct()),
...
Jetzt, wo Sie intuitives Verständnis von der Stichprobenziehung bekommen haben, können wir den Konzepten, die Sie eben gesehen haben, Fachbegriffe zuordnen.
...
::: aufgabe
Berechnen Sie die wahre Proportion roter Kugeln in der Population! 
...
An diesem Punkt könnten Sie sich fragen, wenn wir den wahren Populations-Parameter so einfach bestimmen konnten, warum haben wir dann überhaupt Stichproben gezogen?
