...
Wir fokussieren uns hier darauf das Hantieren mit Data Frames, denn egal wie du die Tabelle erstellt hast - durch Import aus Excel oder Zusammensetzen einzelner Vektoren, es wird immer ein Data Frame in R sein.
...
Um dir einen Überblick über die den Datensatz zu verschaffen, ist es stets ratsam, diesen zunächt anzeigen zu lassen, führe dafür den Codeblock aus.
...
Um gezielt eine bestimmte Spalte anzusteuern, benötigen du also den `$`-Operator.

(Beispiel einfügen)

(Exercise einfügen)

An sich kannst du für die Spalten die gleichen Mechanismen verwenden wie für Vektoren, denn es *sind* Vektoren.

(Beispiel einfügen)

(Exercise)

Du siehst, dass bei Data Frames der Output beim Indizieren mit dem `$`-Operator immer ein Vektor ist.
...
Du siehst, dass du mit `[ , ]` immer einen Data Frame zurückbekommst, während du mit `$` nur den jeweiligen Vektor ausgegeben bekommst der die Spalte darstellt.
...
Lasse dir den Datensatz `net` ausgeben!
...
-   **Spalten hinzufügen:** Die Funktion `cbind()` wurde verwendet, um neue Spalten zu einem Data Frame hinzuzufügen.
