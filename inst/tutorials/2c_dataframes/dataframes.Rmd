...
Wenn du mit Tabellen in R arbeitest - und das wird oft sein - wird es sich meist um *Data Frames* handeln, oder eine spezielle Unterform davon, sogenannte *Tibbles*.
...
## Einführung in Data Frames {data-progressive="TRUE"}
...
### So sieht ein Data Frame aus

Einmal vorweg ein Beispiel für dich, wie ein Data Frame erstellt wird, und vor allem wie er dann aussieht:

Schaue dir die Vektoren `x` und `y` im Codeblock an: Sie enthalten jeweils drei Elemente.
...
Ganz links werden Zeilennummern angezeigt. In der ersten Spalte siehst du dann die Variable `x` und in der zweiten Spalte die Variable `y`. Nun ist jeder Vektor also eine Spalte geworden im Data Frame!
...
::: blau-nb
Eine Konvention für Data Frames ist, als Objektname `df` zu wählen. Da wir jedoch mehr als einen Data Frame erstellen werden, wird in diesem Tutorial davon abgewichen. Es steht dir natürlich auch immer frei, eigene Namen für deine Objekte zu wählen.
:::
...
Eine Matrix eine spezielle Datenstruktur in R, **die immer nur Daten des gleichen Typs enthalten kann**. Sie wird durch den Befehl `matrix()` erstellt. Von der Dokumentation (Hilfe) kannst du erfahren, dass dieser Befehl folgende Argumente ermöglicht: `matrix(data = NA, nrow = 1, ncol = 1)`
