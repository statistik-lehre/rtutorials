...
![](images/matrix_index.png){width="50%"} source:[Base R cheat sheet by Mhairi McNeil](https://iqss.github.io/dss-workshops/R/Rintro/base-r-cheat-sheet.pdf)
...
Ein ganz zentraler Punkt: Während normale Vektoren *atomic* sind, also immer nur Daten des gleichen Typs beherbergen können, können Listen gemischte Daten enthalten und Daten verschiedener Länge.
...
Du siehst die Variablen (Spalten) bekommen doppelte eckige Klammern und die Einträge (Zeilen) einfache eckige Klammern. Wenn wir so auf eine Variable zugreifen wollen wird es schnell unübersichtlich. Wollen wir z.B. auf den Jahrgang von Amadou zugreifen wäre das wg_list[[2]][1]. Probiere es gerne mal aus:
...
Wie gesagt, ist dies etwas mühsam. Listen können jedoch viel mehr! Sie erlauben die Benutzung eines speziellen Operators zur Indizierung: Der `$`- Operator. Er ist speziell für das Indizieren bei benannten Listen gedacht. Hat eine Tabelle überschriften so werden diese übernommen, für unsere Vektoren müssen wir erneut auf die explizite Namensgebung zurückgreifen.
...
```{r dollarsign1, exercise = TRUE, exercise.setup = "listennamen"}
wg_liste$name
wg_liste$jahrgang[3]
```
...
```{r dollarsign2, exercise = TRUE, exercise.setup = "listennamen"}
...
```{r dollarsign3-hint-01}
wg_liste$qm <- c() # erstell einen Vektor in der Liste wg mit den Quadratmeterangaben 
wg_liste$qm[] # greife auf das 2 und 3 Element des Vektors qm der Liste wg_liste zu 
```

```{r dollarsign3-hint-02}
