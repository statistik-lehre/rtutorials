...
Es ist in vielen Teilen ausführlicher als dieses Tutorial und Allen, die
vor englischer Sprache nicht zurückstrecken, wärmstens zu empfehlen dort
...
Das `dplyr`-Paket wird verwendet, um einen Datensatz zu manipulieren. Es
enthält essentielle Funktionen um deine Daten in Form zu bringen:
...
es übrigens ein sehr hilfreiches Feature Texte zu durchsuchen.
:::
...
auf deine eigenen Daten außerhalb dieses Tutorials anwendest.
...
fest, dass ein Überblick deines Datensatzen unübersichtlich ist.
...
geschrieben, die ausgewertet werden können. Zum Beispiel können wir,
...
Ausdruck `len == 22.0`. Es können eine einzelne Bedingung oder mehrere
...
-   `.preserve`: ob wir die Gruppierung des Data Frames erhalten möchten
...
Angenommen, wir wollen für unseren Data Frame `einkaufen` den Weg nicht
nur in km sondern auch in Meter verfügbar haben. Um eine neue Spalte
`weg_m` zu berechnen, könnten wir folgenden Code verwenden:
...
quiz(caption = "Teste dein Wissen zu `mutate()`!",
  ...
  question_radio("Welche Aussage über `mutate()` ist korrekt?",
    ...
    answer("Die durch `mutate()` neu berechnete Variable kann auch *mehr Zeilen* als die Variable aus der sie berechnet wurde", 
         message = "Das die neu berechnete Variable mehr Zeilen hat ist rechnerisch nicht möglich, da sie sich ja aus den bestehenden Werten berechnet"),
  allow_retry = TRUE,
  random_answer_order = TRUE,
  incorrect = random_encouragement("de"),
  correct = random_praise("de")),
...
-   **`ungroup(data)`**: Die Funktion `ungroup()` nimmt **ein Argument**
    entgegen, den gruppierten **Datensatz**, den du aufheben möchtest.
...
weiterhin zufällig. Das liegt daran, dass die Funktion normalerweise in
...
viele neue Gruppen erstellen, das jede Gruppe am Ende aus 1-2 Einträgen
besteht. Einen Mittelwert oder ähnliches hieraus zu berechnen wäre nicht
sinnvoll.
...
dargestellt, sondern als alleinstehende Zeile nach deinen Analysen der
Gruppen den Code so anfügen:
...
Wir bekommen ein NA? Komisch? Nein, erinnere dich daran, dass R sobald
ein NA in der Rechnung beteiligt ist, dir stehts NA als Antwort ausgibt.

Für uns kein Problem, wir sagen der Funktion `sum()` mit dem
Zusatzargument `(na.rm = T)`, dass sie die NAs nicht mit in die Rechnung
einbeziehen soll:
...
langsam die Logik von R ins Blut und die Finger über. 🧙
...
davon ausgehen, dass der Wert nicht zu unserer Population gehört, da
dies als Geburtsjahr nicht möglich ist. Haben wir als Antwort jedoch den
Wert `1920` ist dies zwar ein extremer Wert, aber möglich wäre es
durchaus.
...
Anhaltspunkt und findet viel Anwendung in der Literatur. Daher wollen
auch wir es nutzen, um unseren Datensatz einkaufen zu bereinigen.
...
Fragebogen nach einer bestimmten Zeit abbrechen und somit viele NAs in
unserem Datensatz entstehen. Da diese Fälle für eine Analyse meist
unbrauchbar sind, gibt es auch hier eine Möglichkeit unseren Datensatz
davon zu "reinigen".
...
Doch auch hier ist wieder Vorsicht geboten. Der Schwellen-Wert (hier 50%) sollte im
Vorhinein gut überlegt sein und der **Auschluss der Daten dokumentiert
und bericht** werden!
