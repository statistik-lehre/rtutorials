...
link-citations: TRUE
description: "Ein grundlegender Überblick über den wissenschaftlichen Prozess als solchen mit Orientierung über Dinge, die wir dieses Semester behandeln und optionalem Nachschlagewerk der einzelnen Stationen."
resource_files:
- css/boxes.css
editor_options: 
  markdown: 
...
Auseinandersetzung mit den Daten zwar nicht notwendig, 

<details>

<summary><a>▼ aber sehr hilfreich. **(mit ausführlichem Beispiel)**</a></summary>
...
```{r IV2, message=FALSE, include=TRUE, echo=TRUE}
library(ggplot2)
library(dplyr)
...
filtered_data$nah_fern <- ifelse(grepl("nahverkehr", filtered_data$Typ), "nah", "fern")
...
# sehr schön zu sehen: für den nahverkehr mit omnibussen wurden es mit abstand die meisten Unternehmen erhoben
...
Alle Statistiken, die wir später berechnen wollen haben irgendwelche Voraussetzungen, die sie erfüllen müssen. Diese lassen sich selbstverständlich auch statistisch testen.
...
Erstelle ansprechende und übersichtliche Grafiken, um deine Ergebnisse zu visualisieren, die kommen später auch ins Paper.

S. hierfür Unterkapitel `Visualisierung`.

### VII) Report

Jetzt will das ganze noch in eine schöne Form gebracht werden. Hier werden die Forschungsergebnisse dokumentiert und ein Bericht je nach Anforderungen (Paper, Bachelorarbeit, Studienleistung etc. erstellt). 

## Abschlussquiz

@Marie: bin bisschen ratlos, was ich hier quizzen könnte
