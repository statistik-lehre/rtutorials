...
Wir haben die Zahlenreihe

```{r}
c(1, 2, 3)
```

Als zusammenfassende Zahl nehmen wir einfach mal die 3. 
...
```{r server, context = "server"}
 x <- rnorm(10, mean = 5, sd = 2.5) |> round() |> abs() |> unique() |> sort()
  n <- length(x)
    ...
    output$sum <- renderText({
      paste("Summe der Abweichungen: ", round(sum(x - input$zahl), digits = 3))
      })
```
