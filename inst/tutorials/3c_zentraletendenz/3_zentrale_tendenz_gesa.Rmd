...
## Zentrale Tendenzen {data-progressive=TRUE}
...
Bei einer ungeraden Anzahl von Werten gibt es einen, der genau in der Mitte liegt. Hätten wir nur 9 IQ-Werte, wäre der Median der 5. Wert. Oberhalb liegen 4 höhere Werte und unterhalb 4 niedrigere. 

Wir haben aber 10 Werte, also eine gerade Anzahl.

Bei einer geraden Anzahl von Rängen (wie hier), wo es keine klare Mitte gibt, werden die beiden Werte direkt über und unter der Mitte addiert und durch 2 geteilt. Hier: $\frac{101 + 102}{2} = 101.5$. Der Median ist also 101.5. 
...
Klasse! Das ist genau das gleiche, wie wir ihn auch per Hand berechnet haben. 
...
```
Da es eine nominale Variable ist, ist es wichtig, daran zu denken, sie als `factor()` zu speichern. Um heraus zu finden, welche Gruppe am größten ist, könntest du zählen und eine Strichliste machen. ODER du lässt ganz smooth R für dich arbeiten. Hierfür brauchst du wieder die Funktion `summary()`. 
...
Das wäre in diesem Fall mit 5 Einträgen die Lieblingsfarbe Pink.
```{r modequest}
question_text("Was ist der Modus des Vektors `farben`?",
                 answer("pink", correct = TRUE, message = "Genau, Pink kommt 5 mal vor."),
              allow_retry = TRUE)
```
