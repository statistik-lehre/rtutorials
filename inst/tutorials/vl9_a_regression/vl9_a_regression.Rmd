...
Super! Später können Sie Ihre Schätzungen mit der vom Computer errechneten optimalen Gerade vergleichen.
...
Diese Definition verwendet zur Schätzung der Regressionskoeffizienten die Methode der kleinsten Quadrate (*least squares*).

Günstigerweise liefert diese Methode immer die Gerade mit minimalem Abstand zu den Residuen! 
...
Analog zur Berechnung der Varianz wird hier quadriert. Da quadrierte Werte immer positiv sind, erreicht man, dass sich negative und positive Abweichungen nicht aufheben beim Summieren. Außerdem stellt das Quadrieren weitere günstige mathematische Eigenschaften für das Minimieren her (eine genaue Erklärung führt zu weit).
...
Ableiten von allgemeinen Formeln ist nun möglich. Details führen hier zu weit, aber es gibt gute externe Quellen dazu:
...
#### Exkurs: Fitting durch Machine Learning

Man kann die Quadrierten Residuen auch minimieren durch Trial & Error, wie Sie oben selbst es auch gemacht haben. Das geht super mit Machine-Learning-Algorithmen. 

Hier eine Illustration des Fitting-Prozesses, allerdings mit anderen Daten. Die Grafik zeigt wie ein Machine-Learning-Algorithmus sich Schritt für Schritt der optimalen Regressionsgerade annähert, und die mittlere quadratische Abweichung (Mean Squared Error, MSE) der vorhergesagten Werte von den tatsächlichen Werten dabei immer kleiner wird, bis es nicht kleiner geht.
...
„93,53 % der Varianz im Holzvolumen können auf den Prädiktor "Durchmesser" zurückgeführt werden (lat. *regressare* = zurückgehen).

Allgemein formuliert: 

> „93,53% der Varianz im Kriterium können durch das Modell aufgeklärt werden.“ 

(Zufällig besteht unser Modell nur aus einem Prädiktor, aber in der Praxis werden es häufig mehrere Prädiktoren sein - siehe "Ausblick: multiple lineare Regression".)
...
```{r korrelationsfrage}
question_numeric("Geben Sie Ihren errechneten Wert der Pearson-Korrelation zwischen Volumen und Durchmesser ein, gerundet au 4 Nachkommastellen!",
...
Wir sind sehr kritisch und unterstellen erst mal, dass es in Wahrheit in der gesamten Population keine Varianzaufklärung gibt, und dass unsere Stichprobe nur ein zufällige Abweichung davon ist. Diese Art von kritischer Haltung ist allen Nullhypothesen-Signifikanztests gemeinsam, sei es ein $F$-, $t$- oder $z$-Test.
...
Da $p$ unglaublich klein ist, ist es schwierig, alle relevanten Teile in einem Plot unterzubringen. Deswegen gibt es 3: 

- Übersicht (komplett herausgezoomt)
- Zoom Nr. 1 
- Zoom Nr. 2

In der Übersicht sehen Sie den empirischen (=beobachteten) F-Wert, der sich aus der beobachteten Varianzaufklärung von 93% errechnet. Er entspricht `F-statistic` im R-Output. 

<details>
  <summary><a>▼ \* Wie wird der empirische $F$-Wert berechnet?</a></summary>
  </br>
...
Die Fläche unter der F-Verteilung zwischen 419.4 und $+\infty$ ist der $p$-Wert, aber die ist so klein, dass man in der Übersicht gar keine Fläche sieht, und erst sehr weit hineinzoomen muss (siehe Zoom Nr. 2). Dort kann man den $p$-Wert als Fläche erkennen. 

Die kritische Grenze ist der Punkt, jenseits dessen bis $+\infty$ 5% der Fläche unter der Kurve liegen. Das ist gut zu sehen in Zoom Nr. 1. So lange der empirische F-Wert innerhalb dieses Bereichs bleibt, wird der Test signifikant.
...
Da der Lokaltest eigentlich nur für die multiple lineare Regression wichtig ist, gehen wir nicht im Detail darauf ein, zeigen aber ein paar Besonderheiten. 
