...
1. Erstellen Sie ein Punktdiagramm 
1. Fügen Sie die Regressionsgerade hinzu
...
### Linearer Zusammenhang in den Daten

Manche Daten werden besser nicht durch Geraden erklärt, sondern haben vielleicht quadratische Zusammenhänge:

```{r}
data <- data.frame(Arbeitszeit=c(6, 9, 12, 14, 30, 35, 40, 47, 51, 55, 60),
                   Glück=c(14, 28, 50, 70, 89, 94, 90, 75, 59, 44, 27))


straight_fit <- lm(Glück ~ Arbeitszeit, data = data)
data$Arbeitszeit2 <- data$Arbeitszeit^2
quad_fit <- lm(Glück ~ Arbeitszeit + Arbeitszeit2, data = data)
zeit <- seq(0, 60, 0.1)
prediction <- predict(quad_fit, list(Arbeitszeit = zeit, Arbeitszeit2 = zeit^2))

plot(Glück ~ Arbeitszeit, data = data)
abline(fit, col = "red")
