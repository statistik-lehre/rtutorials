...
```{r shiny_ui, echo=FALSE}
    sliderInput("b0", "Intercept b0:", min = -1.5, max = 1.5, value = 1, step = 0.01)
    sliderInput("b1", "Slope b1:", min = 0, max = 0.15, value = 0, step = 0.001)
    plotOutput("distPlot")
```
...
pre <- input$b0 + input$b1 * trees$Diameter
res <- sum((trees$Volume - pre) ^ 2)
  
ggplot(trees, aes(x = Diameter, y = Volume)) +
  geom_point() +
  geom_abline(intercept = input$b0, slope = input$b1, color = "blue") +
  geom_linerange(x = trees$Diameter, ymin = trees$Volume, ymax = pre, linetype = "solid", colour = "red") +
  annotate(geom = "label",
            x = 50, y = 0.3,
           hjust = 0,
           col = "red",
...
Ein Residuum $e$ der Messung $i$ errechnet sich aus: 
$$e_i = y_i - \hat y_i$$
...
Dahinter steht ein grundlegendes Konzept, die „Methode der kleinsten Quadrate“ (*least squares*), was eine weit verbreitete Schätzmethode ist. Das liegt unter anderem daran, dass das Minimieren der Summe der quadrierten Residuen auch tatsächlich die beste Varianzaufklärung erreicht (siehe Kapitel $R^2$). Zum Vergleich: Warum minimiert man nicht die Summer der absoluten Residuen?
...
Hervorragend, nun möchten Sie sicher wissen, wie die optimalen Regressionskoeffizienten $b_1$ und $b_0$ für unser Modell denn nun lauten. Hang on! Im nächsten Kapitel rechnet R das für uns aus, und Sie können sich einem R-Output stellen!
