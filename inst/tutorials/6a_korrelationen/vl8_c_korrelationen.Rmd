...
Modelle sind wie Werkzeuge, mit denen wir unsere **Daten erklären und Vorhersagen treffen** können. Wenn wir sagen, wir "fitten" ein Modell an unsere Daten, bedeutet das, wir verwenden ein mathematisches Modell, um **Muster in unseren Daten zu erkennen und zu beschreiben**. Erinnere dich an die Streudiagramme und die Korrelationsgerade: Diese Linie ist dein "Modell", das versucht, die Beziehung zwischen den Variablen zu erklären.
...
Diese Beziehungen werden meist als **aufeinander einflussnehmende Boxen** dargestellt. So könnten wir den Einfluss von Sonnencremenutzung auf das Hautkrebsrisiko wie folgt darstellen:

![](images/modell.png){width="90%"}

Du kannst dir bestimmt bereits vorstellen, dass dieses Modell nicht sehr genaue Vorhersagen des Hautkrebsrisikos erlauben wird, da noch weit mehr Wechselwirkungen in der Realität vorliegen. Mir fallen spontan noch ein Mediationseffekt (Hauttyp und Sonnencremenutzung) und ein Moderationseffekt (Wetter beeinflusst den Effekt von Sonnencremenutzung) ein:

![](images/modell2.png){width="90%"}

Falls du dich fragst was Moderation und Mediation ist, findest du [hier eine ausführliche Erklärung](https://statistikguru.de/blog/mediation-und-moderation.html). 

Je mehr relevante Variablen wir also in unser Modell aufnehmen, desto besser können wir die Realität widerspiegeln. Gleichzeitig wollen wir aber auch so wenig Variablen wie nötig inkludieren, um die Komplexität der Realität zu reduzieren, damit wir diese besser verstehen können.

::: gelb
**Modelle** sind also mächtige Werkzeuge, die uns **helfen, Daten zu analysieren und Schlüsse über die Realität zu ziehen**. Aber es ist wichtig zu verstehen, dass sie **Vereinfachungen** sind und nicht die ganze Komplexität der realen Welt erfassen können.
:::
...
-   dass unsere Korrelationsgerade ein Modell für unsere Daten ist
-   Modelle sind essenziell, aber dennoch nur starke Vereinfachungen der Realität
