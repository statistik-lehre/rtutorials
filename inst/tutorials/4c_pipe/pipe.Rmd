...
data.frame(einkaufen$Weg, einkaufen$Verkehrsmittel, einkaufen$Essen)
...
oder du nutzt das eingebaute Tastaturkürzel `Strg/Cmd` + `Umschalt` + `M` nutzen.
...
question_text("Was ist der Mittelwert für den Weg zum Einkaufsladen auf zwei Nachkommastellen gerundet?",
                 answer("12.76", correct = TRUE, message = "Genau, im Durchschnitt beträgt die Weglänge zum",
                 "Einkaufsladen 12.76km."),
              allow_retry = TRUE)
```
...
::: blau-nb
Aus dem Packet `dplyr` kommt die hilfreiche Funktion `filter()`. Diese können wir nuzten, um bestimmte Daten aus unserem Datensatz zu filtern.

Die Funktion erwartet als Argumente den `Datensatz`, die `Variable` mit `Operator` und filterndem `Wert`.

filter(Daten, Variable Operator Wert)

für unser Beispiel:
filter(einkaufen, Essen == "Pizza")

würde uns nur noch die Einträge anzeigen, bei denen die Befragten als Lieblingsessen Pizza angegeben haben.
:::
...
Schau dir die Funktionsweiese der Funktion `filter()` an und nutze sie für diese Aufgabe.
Füttere die Daten von `einkaufen` in die Funktion `filter()` und filtere dort in der Variable `Essen` nach den Werten für "Pasta". Gib die Daten in die Funktion `count()`, um dir die Anzahl ausgeben zu lassen.
:::
...
question_text("Wie viele Menschen haben Pasta als Lieblingessen angegeben?",
                 answer("13", correct = TRUE, message = "Genau, 13 Befragte essen am liebsten Pasta."),
              allow_retry = TRUE)
```
...
learnr::question_checkbox("Welche der folgenden Aussagen sind korrekt? Markiere alle zutreffenden Aussagen.",
         answer(paste("Der Pipe-Operator ist in deinem Code nicht erforderlich, aber es ist hilfreich,",
                      " weil er den Code sauberer macht."), correct = TRUE),
         answer(paste("Der Pipe-Operator gibt den Output als erstes Argument in die nachfolgende Funktion ein"),
                correct = TRUE,
                message = "Genau. Die meisten Funktionen nehmen Daten als erstes Argument. Diese Funktionen",
                "können wir also mit Daten aus der Pipe füttern"),
         answer(paste("Der Pipe-Operator kann sowohl Data Frames als auch Funktionen",
                      " in andere Funktionen einleiten."),
                message = "Der Pipe-Operator füttert nur Daten in Funktionen."),
         allow_retry = TRUE,
...
learnr::question_checkbox("Was ist der Unterschied zwischen dem `%>%` und dem `|>` Pipe Operator?",
         ...
         answer("Die `|>` Pipe kann nur genutzt werden, wenn das `magrittr`-Paket geladen wurde.",
              message = "Die `|>` Pipe ist in `Base R` implementiert und muss nicht extra aus",
              "einem Paket geladen werden.",
              correct = TRUE),
         answer("Die `%>%` Pipe kann nur genutzt werden, wenn das `magrittr`-Paket geladen wurde.",
