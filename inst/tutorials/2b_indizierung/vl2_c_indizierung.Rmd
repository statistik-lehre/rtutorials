...
Innerhalb der eckigen Klammern wird ein Vektor angegeben. In diesem Fall die Zahlenreihe `1:10`. Diese wählt *Element an Positon 1* bis *Element an Positon 10* aus. 
...
x[-(11:20)]   # schließt Messung 11 bis 20 aus. 
```
:::
...
Sehr gut! In diesem Abschnitt hast du gelernt, wie Indizierung nach Position der Elemente funktioniert. Das ist nur einer von drei möglichen Wegen. Im nächsten Abschnitt wirst du lernen, wie Indizierung nach Inhalt (Wert) der Elemente funktioniert.

## Indizierung nach Werten

Im vorherigen Abschnitt hast du Zahlenvektoren verwendet, um die Position der gewünschten Elemente anzugeben. Wie du im Tutorial "Vektoren" gelernt hast, gibt es logische Vektoren. So sieht zum Beispiel einer aus:
...
```{r condiquest}
question_radio("Ist der `!=`-Operator eine vektorisierte Funktion?",
               answer("Ja",
                      correct = TRUE,
                      message = "Richtig, da die Prüfung auf jedes Element des gesamten Vektors angewendet wird, und auch als Ergebnis wieder ein Vektor mit der gleichen Länge wie der Input steht."),
               answer("Nein",
                      message = "Leider falsch, da die Prüfung automatisch auf `!=` jedes Element im Vektor `x` angewandt wird."),
               allow_retry = TRUE)
```
...
# Erstellung eines Beispielvektors 
temp <- c(23, 26, 24, 28, 22, 30, 18, 27, 25, 22, 0, 45, 2, 5, -2, 11, 16, 42)
temp

# Subset erstellen: Temperaturen zwischen 25 und 30
temp_subset <- temp[temp > 25 & temp < 30]
temp_subset
```
...
::: blau-nb
Ein Subset ist dabei nichts anderes als eine Teilmenge von Daten, die durch die Auswahl bestimmter Elemente aus einem größeren Datensatz gebildet wird. In unserem Fall ist `temp_subset` also ein Subset von `temp`. Dank des Subsets ist es dann möglich, nur mit einer Teilmenge der ursprünglichen Daten weiterzurechnen. 
:::
...
::: aufgabe
Verwende den Vektor `temp` und lasse dir alle Temperaturen über 26 sowie unter 20 mit Hilfe des ODER-Operators ausgeben.
:::
...
```{r Ops1-solution}
# Vom Vektor temp nimm alle Temperaturen, die unter 20 ODER über 26 liegen
temp[temp > 26 | temp < 20]
...
```{r Ops2-solution}
# alle Temperaturen, die unter 20 UND über 26 liegen
temp[temp > 26 & temp < 20]

# Output:
# numeric(0): Ein numerischer Vektor der Länge Null.

# Das Ergebnis ist ein leeres Subset, 
# da die Temperatur nicht gleichzeitig über 26 
# und unter 20 sein kann (zumindest nicht in einem einzelnen Universum).
```
...
::: grau-nb
Hinweis: Falls du dich fragst, warum beim Wert von `y` ein `L` dahinter steht: Das ist völlig normal. Das `L` steht für *Long* und signalisiert einen Datentyp, nämlich `integer`. 
:::
...
question_checkbox("Wie wählt man die Elemente 5 bis 8 eines Vektors aus?",
         answer("`vector[5, 8]`"),
         answer("`vector[c(5, 8)]`",
                message = "`c(5, 8)` gibt nur das 5. und das 8. Element aus. "),
