...
Das ist wichtig zu erwähnen, denn das ist krass: Computer merken sich Zahlen anders als wir Menschen. Menschen denken *dezimal* (also in Zehnerschritten) und Computer *binär* (die klassischen 1 und 0). Für den Menschen sieht eine `0.1` so aus: 0.1 bzw. $$\frac{1}{10}$$ Für einen Rechner sieht eine `0.1` so aus: $$0.0 \overline{0011}$$. Rechner können je nach Programmeinstellung und in sehr hardcore Fällen auch aufgrund der Hardware pro Zahl nur  [23](https://de.wikipedia.org/wiki/Einfache_Genauigkeit) bzw. [52](https://de.wikipedia.org/wiki/Doppelte_Genauigkeit) binäre [signifikante Stellen](https://de.wikipedia.org/wiki/Signifikante_Stellen). Das entspricht ca 8 bzw. 16 dezimalen Ziffern (*Siehe dazu auch dieses wunderbare [Video](https://www.youtube.com/watch?v=PZRI1IfStY0)). Danach wird die Zahl einfach abgeschnitten - mit fatalen Folgen.
...
Ein Vektor ist die einfachste Datenstruktur. Es ist quasi eine Aneinanderreihung von Zahlen. Einfache Zahlen werden als Vektor mit einem Eintrag gewertet. Wenn die Daten in einem Vektor Objekt organisiert sind, wirft die Funktion `class()` im groben und ganze den Typen der darin gespeicherten Daten aus.

:::aufgabe
Finde für die aufgeführten Vektore de Datentyp und Objektklasse heraus. Mach dich mit den Gleichheiten und Unterschieden vertraut.
:::

```{r class1, exercise = TRUE}
# im Folgenden werde aus oben genannten Gründen nur die vier Häufigsten Datentypen besprochen
...
```{r class1-solution, setup.chunk = "class1"}
...
Tatsächlich liegt die unterschiedliche Antwort in der unterschiedlichen Frage: `typeof()` fragt nach dem grundlegenden **Datentyp** (also wie ist die Zahl im Rechner repräsentiert? $\to$ In diesem Fall eben als Fließkommazahl mit doppelter Genauigkeit) während `class()`die Klasse des Objekts anfragt, die angibt, wie R damit umgehen soll. (Das ist eine gröbere Unterteilung, es reicht die Info, dass es numerische Daten sind (und eben keine Buchstaben oä)).

Und was ist bei gemischten Vektoren?
...
<details>
<summary><a>▼ \* Für alle, dies wissen wollen.
</a></summary>

Factoren in R sind ne feine Angelegenheit. Auf den ersten Blick recht kompliziert und unintuitiv ermöglichen sie dem geübten Händchen vor allem hintenraus doch eine schnelles und einfaches Arbeiten. **Bitte trotz gleichem wording im Kopf konzeptuell trennen von dem statistischen Konzept Faktor aus der Faktorenanalyse, das ist was grundlegend anderes!** (Deswegen wird in diesem Kurs der Factor in R auch mit c geschrieben.)

Ein Factor ist genau genommen ein Spezialfall eines Vektors für **kategoriale** Daten (also denen, die sich in nicht hierarchisch sortierbare Gruppen einteilen lassen (z.B. Farben, Städte, Studiengang etc.)) und **ordinale** Daten (also denen, wo sich die Gruppen zwar hierarchisieren lassen, die Abstände aber nicht interpretierbar sind (z.B. Rangfolgen von Lieblingssüßigkeiten, Filmen etc., Bildungsniveau, Plätze bei einem Sportwettkampf etc.)).
...
```{r class_fac2, exercise = TRUE, setup.chunk = "class_fac1"}
...
#### Zwei-dimensionale Objekte: Data Frames, Matrizen, Tibbles

2 Dimensionen in Daten heißt: Zeilen UND Spalten. Also im Grunde ist jede klassische Tabelle.

R unterscheidet: Data Frames, Matritzen und Tibbles.

**Data Frames**: Sind die mehrdimensionale Datenstruktur, die dir am häufigsten begegnen wird, mit der eigentlich standardmäßig gearbeitet wird. Wie eine Tabelle. Im Grunde mehrere nebeneinander geklebte Vektoren. Sie werden folgendermaßen erstellt:
...
Das `list` weist darauf hin, dass es verschiedene Datentypen in diesem Objekt gibt. Die Objektklasse `data.frame` ist genau die erwartete.
...
**Tibbles** sind eine sonderangelegenheit von R, die im Tidyverse mitgeliefert werden. Es sind quasi Data Frames nur in gemütlich.
...
**Listen** sind grob gesagt mehrdimensionales Dataframes - es können also mehrere Dataframesin einem Objekt gespeichert werden. Die sind besonders wichtig, wenn das Arbeiten mit R noch mehr ins programmieren geht und Schrittweise Dinge abgespeichert werden sollen, sehr komplexe Daten berechnet werden oder auch häufig die Ausgabe von komplexeren Funktionen. Der Einfachheit halber



```{r class_setup, include = FALSE}
data_df <- data.frame(
  # Variablennamen für die erste Variable festlegen; hier folgt ein Gleichzeichen, kein Pfeil; Inhalt festlegen
  namen = c("Joh", "Ma", "Lio"),
  weg_km = c(1, 5, 2),
...
```{r class_lis, exercise = TRUE, setup.chunk = "class_setup"}
data_lis <-list(
  df_alt = data_df,
...
Um auf einzelne Listenelemente zugreifen zu können schaue gern auch in das Tutorial Indizierung.
...
Das waren die am häufigsten vorkommenden Objektklassen.
...
| `as.data.frame()` | Umwandlung in data frame |
