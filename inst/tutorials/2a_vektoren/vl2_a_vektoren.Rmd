...
::: aufgabe
Schreiben Sie ein Programm, das den Textvektor "Hello World!" als Ergebnis ausgibt! 
:::
...
Falls Sie über den Fehler:
`non-numeric argument to binary operator` stolpern, untersuchen Sie ihre
Berechnung auf solche Probleme.

::: aufgabe
Ändern Sie den Code, so dass die Berechnung fehlerfrei ausgeführt werden kann!
:::
...
# es gibt auch andere Lösungen, z.B. "vier" ganz zu entfernen und durch einen fehlenden Wert `NA` zu ersetzen. Das würde auch eine Funktion tun, die automatisch versucht, Textvektoren in numerische Vektoren umzuwandeln: 
as.numeric(id)
# Diese Funktion haben wir aber noch nicht vorgestellt.
```
...
Sie haben gelernt, wie Sie Variablen Daten zuweisen. Variablen sind in R repräsentiert als Vektoren. Vektoren sind der fundamentale Objekttyp in R, auf dem alle anderen Objekte wie z.B. Tabellen aufbauen. Ihr Vorteil ist, dass sie mehrere Werte enthalten können.

Beim Rechnen mit Vektoren haben Sie das Konzept "Recycling" kennengelernt,und erahnen ungefähr, was "Vektorisierung" ist - dass vektorisierte Funktionen wie `+` auf jedes Element eines Vektors einzeln angewendet werden, und als Ergebnis auch wieder ein Vektor steht.

Beim Assigment haben Sie gelernt, Leerzeichen zu verwenden um den Assignment Operator <-`. 
