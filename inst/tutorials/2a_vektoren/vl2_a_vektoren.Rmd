...
Puh! Die Zahlentypen sind geschafft! Das war das komplizierteste (und spannedste). Folgen noch die anderen.
...
Daten des Typen `complex` sind, wie der Name schon sagt: [Komplex](https://de.wikipedia.org/wiki/Komplexe_Zahl). Das bedeutet: [Reelle](https://de.wikipedia.org/wiki/Reelle_Zahl) plus [imaginäre](https://de.wikipedia.org/wiki/Imagin%C3%A4re_Zahl) Zahlen. Also Zahlen, die sich nicht durch ganzzahlige Brühe darstellen lassen. Die bekannstesten sind wohl: $\Pi$, $e$ und $i$. Also die [Kreiszahl](https://de.wikipedia.org/wiki/Kreiszahl), die [eulersche Zahl](https://de.wikipedia.org/wiki/Eulersche_Zahl) und die [imaginäre Zahl](https://de.wikipedia.org/wiki/Imagin%C3%A4re_Zahl). In R: `pi`, `exp()` und `0i`. Damit werdet ihr so gut wie nie konfrontiert werden.
