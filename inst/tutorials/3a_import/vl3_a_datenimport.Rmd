...
Das hier wäre zum Beispiel ein typischer Windows-Pfad: `C:\User\Nina\Documents\R\Daten\einkaufen.csv`

Und so sieht ein typischer Mac-Pfad aus:
`~/Nina/Documents/R/Daten/einkaufen.csv`

Windows verwendet also `\` Backward-Slashes, während Mac und Linux immer `/` Forward-Slashes verwenden! Arme Windos-User, denn R akzeptiert keine Backward-Slashes.

Nanu, was ist denn mit dieser Tabelle hier los? Offensichtlich wurde das Semikolen nicht als Trennzeichen erkannt, sondern R hat nach einem Komma gesucht, um die Spalten voneinander zu trennen, aber keins gefunden! Alles wurde in eine Spalte gezwängt.

```{r zerhaeckselte_tabelle, echo = TRUE}
# path <- "/speicherort/von/einkaufen.csv"
read.csv(path)
```

```{r cleane_tabelle, echo = TRUE}
read.csv2(path)
```
