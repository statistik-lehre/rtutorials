...
Willkommen im Tutorial zum Importieren von Daten in R! In diesem Abschnitt lernst du, wie du deine Daten in R lädst, um sie für deine Analysen und Visualisierungen zu verwenden. Der Fokus liegt dabei vor allem auf dem Import von CSV- und Excel-Dateien, zwei der gängigsten Formate für den Datenaustausch.
...
Diese Dateien erkennt man an der Endung `.csv`, was *comma separated values* bedeutet. Schauen wir uns mal an, wie so eine CSV-Datei aus der Nähe aussieht, wenn wir
sie als reinen Text anzeigen lassen. Direkt darunter zum Vergleich, wie die Daten als Tabelle aussehen. 

```{r}
path <- system.file("extdata", "einkaufen.csv", package = "rtutorials")
txt <- readChar(path, nchars = 432)
cat(txt)
head(einkaufen)
```
...
- Eine neue Zeile im Text ist eine neue Zeile in der Tabelle
- Die erste Zeile enthält die Spaltennamen, also den Tabellenkopf (engl. *header*)
- Die einzelnen Werte sind mit einem `;` getrennt! `;` ist das *Trennzeichen* in diesem Fall. 


::: infobox
**Gut zu wissen:**

Im englischen Raum wird oft das Komma `,` als Trennzeichen verwendet, was den Namen CSV: *comma separated values* erklärt. In Deutschland würde das Komma zu Konflikten mit dem Dezimaltrenner `,` führen, weswegen dort meist das Semikolon `;` als Trennzeichen verwendet wird!
Es heißt aber trotzdem noch CSV-Datei. 

Das Trennzeichen ist die häufigste Ursache, wenn CSV-Dateien komisch oder zerhäckselt eingelesen werden.

Trennzeichen heißt auf Englisch übrigens *delimiter*. 
:::

### Das Einlesen

Der Prozess, aus dem rohen Text eine Tabelle zu konstruieren, heißt Einlesen oder Datenimport. Die Daten müssen in eine Tabellenform, damit wir (und R) damit weiterarbeiten können. 

Für CSV-Dateien übernimmt das Einlesen die Funktion `read.csv()`.

#### Dateipfade

Hier noch ein kleines Wort zu Dateipfaden. Für diejenigen, die nicht wissen, was das ist: Das ist eine Adressangabe für den Computer, wo sich eine bestimmte Datei befindet.

Dateipfade sind schmerzvoll und nervig, weil sie je nach Betriebssystem unterschiedliche Formate haben. 

Das hier wäre zum Beispiel ein typischer Windows-Pfad: \ `C:\User\Nina\Documents\R\statistik-bachelor\einkaufen.csv`

Und so sieht ein typischer Mac- oder Linux-Pfad aus: \
`~/Nina/Documents/R/statistik-bachelor/einkaufen.csv`

Windows verwendet also `\` Backward-Slashes, während Mac und Linux immer `/` Forward-Slashes verwenden! Arme Windos-User, denn R akzeptiert keine Backward-Slashes. Da gehen die Probleme schon los. Deswegen versuchen wir den direkten Kontakt mit *absoluten* Dateipfaden möglichst zu vermeiden, indem wir

- grafische Benutzeroberflächen zum Importieren von Daten verwenden
- mit Projekten arbeiten, so dass der Pfad nur noch so aussähe: `einkaufen.csv` (vorausgesetzt, man befindet sich im aktiven Projekt `statistik-bachelor`). Dieses Konzept heißt auch *relative Pfade* und sollte immer bevorzugt werden, da Skripte dann funktionieren unabhängig vom PC, an dem du dich gerade befindest.

Um hier im Tutorial fortfahren zu können, haben wir das Dateipfad-Problem mal beiseite gelegt, in dem wir den Textvektor `path` verwenden. Der wurde im Hintergrund von einem Skript erstellt enthält einen Pfad zu einer CSV-Datei, die mit diesem Paket zusammen auf deinen PC heruntergeladen wurde. Mit dieser Datei werden wir im Folgenden üben. 

::: aufgabe
Der Codeblock ist schon ausgefüllt, damit du sehen kannst, wie es funktioniert:

1. Lasse dir `path` ausgeben, um zu sehen, wie ein Dateipfad auf deinem System aussieht.

2. Lies die CSV-Datei an diesem Dateipfad ein mit `read.csv()` 
:::
...
```{r readcsv, exercise=TRUE, exercise.setup = "path", exercise.cap = "CSV einlesen"}
# 1.
path
...
Nanu, was ist denn mit dieser Tabelle hier los? Offensichtlich wurde das Semikolon nicht als Trennzeichen erkannt, sondern R hat nach einem Komma gesucht, um die Spalten voneinander zu trennen, aber keins gefunden! Alles wurde in eine Spalte gezwängt.

::: aufgabe

Versuch mal `read.csv2()` zum Einlesen der Tabelle! Das ist eine Funktion, die standardmäßig das Semikolon als Trennzeichen interpretiert.

:::

```{r read2, exercise = TRUE, exercise.cap = "CSV einlesen mit Semikolon", exercise.setup = "path"}
read.csv2(path)
```

Perfekt! Das sieht schon viel mehr nach einer Tabelle aus.

Jetzt erinnere dich an das Tutorial zu Assignment und Vektoren: Alles, was nicht benannt wird mit `<-` ist *printed and lost*... Wollen wir die Tabelle also nicht nur ausgeben lassen, sondern für die weitere Verwendung im Environment speichern, musst du `<-` verwenden. 

::: aufgabe
Lies die Tabelle ein und speichere das Ergebnis unter dem Namen `df` (wie Data Frame)!
:::


```{r assign, exercise = TRUE, exercise.cap = "CSV einlesen und speichern", exercise.setup = "path"}
read.csv2(path)
```
...
Wir gehen im Folgenden an einem Beispiel durch, wie eine Excel-Tabelle importiert werden kann. 
...
verwenden willst, tippst du einfach folgendes: `rtutorials::datensatz` oder lädst das Paket `rtutorials` und kannst dann die enthaltenen Datensätze ohne die Adresse `rtutorials::` verwenden. 

::: aufgabe

Lade das Paket `rtutorials` in die aktive Sitzung, damit wir es im weiteren Verlauf verwenden können!
...
Wir werden in diesem Tutorial auf den Datensatz `einkaufen` zurückgreifen, welcher Daten enthält die 2022 von Studis im Rahmen ihrer Felderhebung erhoben wurden. Als Thema haben sie sich damals "Bioläden" ausgesucht. 

::: aufgabe

Verwende die Funktion `dplyr::glimpse()`, um einen schnellen Überblick über den Datensatz `einkaufen` zu erlangen!
...
**Kleine Wiederholung:**
```{r wiederholung}
question_checkbox("Klicke alles an, was richtig ist!",
                  answer("`::` signalisiert, in welchem Paket R nach einem bestimmten Objekt (Funktion, Datensatz...) suchen soll", correct = TRUE),
                  answer("`::` ist eine gute Alternative zu `library()`, wenn nur schnell mal eben ein einziges Objekt (Funktion, Datensatz...) aus einem Paket gebraucht wird.", correct = TRUE),
                  random_answer_order = TRUE,
                  allow_retry = TRUE)
```


Sobald du aber das warme Nest dieses Tutorials verlässt, wirst du
