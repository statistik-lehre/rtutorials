...
Einige Annahmen beziehen sich auf die Residuen und können erst nach dem Modellieren überprüft werden. Die Residuen sollten:
...
Tests, insbesondere bei der linearen Regression, auf dieser Annahme bauen. Das Gegenteil von Homoskedastizität heißt Heteroskedastizität. also ungleich verteilte Varianz, und kann zu
...
Die Fehlerterme (Residuen) sollen rein zufälliger Natur sein, und keine systematischen Einflüssen enthalten. Das lässt sich ebenfalls gut mit `plot(fit, which = 1)` visuell betrachten. Generell ist alles schlecht, was die Fehler systematisch beeinflusst. Mit dem Plot der Residuen vs. vorhergesagten Werte kannst du also noch mal nachträglich prüfen, ob die Residuen:
...
Hier mal zwei Plots, wobei der erste unabhängige, homoskedastische, lineare Residuen darstellt, während beim zweiten Plot die Residuen systematisch beeinflusst sind 
...
Der erste Plot zeigt die gewünschte gleichmäßige Verteilung. Bei einigen weit außen liegenden Werten wurde eine Beschriftung angefügt, um sich diese Datenpunkte eventuell genauer anzusehen. Idealerweise sollte die (schwer zu erkennende) rote Linie horizontal auf der Höhe von 0 verlaufen.

Beim zweiten Plot ist gut zu erkennen, dass Residuen (Abstand zwischen Vorhersage und tatsächlichem Wert) systematisch zu groß oder zu klein sind. Beispielsweise sind für den vorhergesagten Wert 0.10 alle Residuen zu hoch, während für 0.20 alle Residuen zu niedrig sind. Das ist ein schlechtes Zeichen, was z.B. auf einen **nicht-linearen** Zusammenhang hindeuten kann, oder dass es zusätzliche Einflüsse gibt, die nicht im Modell enthalten sind, das heißt die Residuen sind nicht **unabhängig**.

### 6. keine Auto-Korrelation der Residuen

Das ist einfach noch ein weiterer Weg um die Unabhängigkeit der Residuen zu überprüfen. 
...
sein müssen. Sie entsteht aus der Annahme, dass die Residuen rein zufällig zustande kommen.
...
question_radio("Ist die Vorannahme erfüllt, dass die Residuen keine Autokorrelation vorweisen? ($\\alpha = .05$)",
                  ...
                  answer("Nein.", 
                        correct = T, 
                        message = "Genau, da $p = .018 < \\alpha = .05$ können wir die Nullhypthese verwerfen, dass es keine Autokorrelation gibt."),
                  allow_retry = T,
               correct = random_praise("de"),
               incorrect = random_encouragement("de")
)
