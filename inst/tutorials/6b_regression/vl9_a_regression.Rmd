...
link-citations: TRUE
description: "Einführung in die einfache lineare Regression und die Überprüfung der Voraussetzungen. Interpretation des Outputs und darüber hinaus viele interessante Nebeninformationen und Exkurse. Wiederholung Signifikanztests. Beachte: Das Laden dieses Tutorials bei der ersten Ausführung dauert lange (ca. 60 Sekunden) und das ist normal."
resource_files:
- css/boxes.css
tutorial:
  id: reg
...
in R geprüft werden können. Disclaimer: Das Thema füllt ganze Bücher, das hier ist nur das Wichtigste. 
...
| Vorannahme                       | Überprüfung                       |
+==================================+===================================+
| 5.  Homoskedastizität            | `plot(fit, which = 1)`            |
|     und Unabhängigkeit der       |                                   |
|     Residuen                     |                                   |
+----------------------------------+-----------------------------------+
| 6.  keine Autokorrelation der    | `car::durbinWatsonTest(fit)`      |
|     Residuen                     |                                   |
+----------------------------------+-----------------------------------+
| 7.  Normalverteilung der         | Q-Q-Plot: der zweite Plot bei     |
|     Residuen                     | `plot(fit, which = 2)`            |
|                                  |                                   |
|                                  | Histogramm der                    |
...
Einige Annahmen beziehen sich auf die **Residuen** und können erst nach dem Modellieren überprüft werden.
...
### 5. Homoskedastizität und Unabhängigkeit der Residuen

**Homoskedastizität**

Wieder mal ein langes und kompliziertes Wort, das dir da begegnet. Aber
nicht verzagen, es meint lediglich: **konstante Varianz**. Das bedeutet, dass die Streuung der
Residuen um die Regressionsgerade für alle Werte der unabhängigen
...
Die Einhaltung der Homoskedastizität der Residuen ist wichtig, da viele statistische
Tests, insbesondere bei der linearen Regression, auf dieser Annahme bauen. Heteroskedastizität, also ungleich verteilte Varianz, kann zu
ungenauen Schätzungen und zu irreführenden Testergebnissen führen.

Schau dir die Grafiken an und versuche zu
...
**Prüfung mit `plot(fit, which = 1)`**

Die Fehlerterme (Residuen) sollen rein zufälliger Natur sein, und keine systematischen Einflüssen enthalten. Das lässt sich gut mit `plot(fit, which = 1)` visuell betrachten. Dieser Plot zeigt die Residuen in Beziehung zu den vorhergesagten Werten. Also grob gesagt, um wie viel das Modell sich verschätzt, je nach dem ob es um die Vorhersage 1 m³ Holz geht oder um 30 m³. Idealerweise wäre die Vorhersage gleich ungenau für alle Werte. Ein Aspekt davon ist *Homoskedastizität der Residuen*. Die Fehler in der Vorhersage sollen rein zufällig ein, und nicht systematisch zu groß oder zu klein für bestimmte Werte. Wünschenswert ist ein Modell, dass über den gesamten Wertebereich gleichmäßig gute Vorhersagen liefert, und dass alle Abweichungen von den Vorhersagen als Zufall betrachtet werden können. Zumindest ist genau das die Grundannahme der linearen Regression.  

**Unabhängigkeit der Residuen**

Hier mal zwei Plots, wobei der erste unabhängige, homoskedastische, lineare Residuen darstellt, während beim zweiten Plot **systematische Fehler** in der  Vorhersage auftreten. 
...
Der erste Plot zeigt die gewünschte **gleichmäßige Verteilung**. Bei einigen weit außen liegenden Werten wurde eine Beschriftung angefügt, um sich diese Datenpunkte eventuell genauer anzusehen. **Idealerweise** sollte die (schwer zu erkennende) **rote Linie horizontal** auf der Höhe von 0 verlaufen. Die rote Linie ist ein Mittelwert der umliegenden Residuen. Wenn der Mittelwert der Residuen 0 lautet, dann weichen sie gleich stark nach unten und nach oben ab, und im Mittel trifft die Vorhersage dann die Realität. Wenn die Residuen größer als 0 sind, ist die Vorhersage niedriger als die Realität. Und umgekehrt, wenn die Residuen kleiner als 0 sind, überschätzt das Modell den realen Wert. Beides ist nicht wünschenswert.

Beim zweiten Plot ist gut zu erkennen, dass Residuen (Abstand zwischen Vorhersage und tatsächlichem Wert) **systematisch zu groß oder zu klein sind**. Beispielsweise sind für den vorhergesagten Wert 0.10 alle Residuen zu hoch, während für 0.20 alle Residuen zu niedrig sind. Das ist ein schlechtes Zeichen, was z.B. auf einen **nicht-linearen** Zusammenhang hindeuten kann, was in diesem Fall auch die Erklärung ist, da die Originaldaten einer Sinuskurve folgen und nicht geeignet sind mit einem linearen Modell beschrieben zu werden. In der Praxis können solche Muster entstehen, wenn es zusätzliche Einflüsse gibt, die nicht im Modell enthalten sind, das heißt die Residuen sind nicht **unabhängig**.

### 6. keine Auto-Korrelation der Residuen
...
eines Hypothesentests, der die Nullhypothese *rho* = 0 testet. 
...
Homoskedastizitäts-Beispielen, die Residuen variieren systematisch!
...
<!-- Zur Frage oben: Warum da \\alpha steht statt \alpha: In Character Strings wird backslash als Escape-Character geparst und muss daher von einem weiteren backslash escaped werden, um in $$ an LaTex übergeben werden zu können. -->
...
Ein idealer, rein zufälliger Messfehler ist normalverteilt, und genau das sollen die Residuen auch sein. Sind die Residuen nicht normalverteilt, deutet das wieder darauf hin, dass sie noch von anderen Dingen außer dem Zufall abhängen.

Eine Möglichkeit, die Normalverteilung grob visuell zu überprüfen, ist ein Histogramm der 
...
```{r residuenhist, exercise = T, exercise.setup = "silentsetup"}
resid(fit) |> scale() |> hist()
```
...
<br>
**QQ-Plot**
Natürlich kannst du die Normalverteilung auch mit einem Quantil-Quantil-Plot visuell prüfen. Dabei werden die Quantile der Daten mit den theoretischen Quantilen der Normalverteilung abgeglichen. Liegen die Datenpunkte auf der diagonalen Linie, stimmen die Quantile der Daten mit den Quantilen einer Normalverteilung überein. Den QQ-Plot von Residuen eines Modells erreichst du auch über die `plot()`-Funktion, mit:
...
```{r qqplot-question}
question_radio(
  "Würdest du die Daten als annähernd normalverteilt bezeichnen?",
  answer("Ja", correct = T),
  answer("Nein"),
  message = "Die meisten Punkte liegen entlang der eingezeichneten Diagonalen.",
  correct = random_praise("de"),
  incorrect = random_encouragement("de"),
  allow_retry = T,
  random_answer_order = T
...
Jetzt können wir testen, ob das Gewicht als Prädiktor einen Teil der
Varianz in der Reichweite erklären kann. Anders formuliert: ob die Reichweite als lineare Funktion des Gewichts modelliert werden kann. 
...
quiz(
  ...
  question_numeric(
    ...
    correct = random_praise("de"),
    incorrect = random_encouragement("de"),
    text = "Wie lautet $b_0$ (Achsenabschnitt)?",
    answer(37.21, correct = T),
    tolerance = 0.3
    ),
  question_numeric(
    allow_retry = T,
    correct = random_praise("de"),
    incorrect = random_encouragement("de"),
    text = "Wie lautet $b_1$ (Regressionsgewicht d. Prädiktors)?",
    answer(-5.3445, correct = T),
    tolerance = 0.1
    )
  )
```
...
Plotten wir doch zuest unser erstelltes Modell, um die **Homoskedastizität** und die **Ünabhängigkeit** der Residuen zu überprüfen. Dafür kannst du in der *Base R* Funktion `plot()` einfach den Modellnamen `fit` eingeben und das Modell wird visualisiert. (Mit dem zusätzlichen Argument `which` lassen wir uns nur die ersten beiden Plots ausgeben).
...
Hier siehst du die Residuen (Vorhersagefehler) abgetragen gegen die vom Modell vorhergesagten Werte (*Fitted*) für die Reichweite. 
...
**$H_0:$ Es liegt Homoskedastizität vor.**
...
**Plot 2: Normalverteilung prüfen mit QQ-Plot**
...
question_radio("Ist die Vorannahme erfüllt, dass die Residuen keine Autokorrelation vorweisen? Es gilt: $\\alpha = .05$",
                  answer("Ja.", 
                         correct = F, 
                         message = "Der $p$-Wert des Durbin-Watson-Tests ist kleiner als .05 und somit nicht signifikant"),
                  answer("Nein.", 
                        correct = T, 
                        message = "Da $p = .014 < \\alpha = .05$ müssen wir die Nullhypthese verwerfen, dass es keine Autokorrelation gibt."),
                  allow_retry = T,
               correct = random_praise("de"),
               incorrect = random_encouragement("de")
)
...
question_radio("Ist die Vorannahme erfüllt, dass die Residuen (annähernd) normalverteilt sind?",
                  answer("Ja!", 
                         correct = T, 
...
question_radio("Würdest du sagen, das Hinzufügen des Prädiktors „Höhe” hat die Anpassungsgüte $R^2$ des Modells signifikant verbessert?",
...
question_radio("3. Was wird im Gesamtmodelltest getestet?",
    ...
    incorrect = random_encouragement("de")),
question_radio(
  "4.* Wann entspricht der Lokaltest dem Geamtmodelltest?",
  answer("Nur bei der einfachen linearen Regression", correct = T),
  answer("Nur bei der multiplen linearen Regression"),
  message = "Da es bei der einfachen Regression nur einen Prädiktor gibt, entspricht der Test für diesen einen Prädiktor dem Test für das gesamte Modell",
  allow_retry = T,
  random_answer_order = T,
  correct = random_praise("de"),
  incorrect = random_encouragement("de")
