...
CRAN steht für *Comprehensive R Archive Network* und ist die offizielle Adresse für geprüfte Erweiterungen. Es sei aber erwähnt, dass es auch noch andere Orte gibt, von denen man Pakete beziehen kann, und deswegen ist die Gesamtzahl aktiv gepflegter Pakete wohl noch höher als 20.000.
...
tbl <- rvest::read_html("https://cran.r-project.org/web/packages/available_packages_by_date.html")
df <- rvest::html_table(tbl)
df <- df[[1]] 
df2 <- df |> dplyr::select(Date, Package) |> dplyr::arrange(Date)
df2$id <- 1:nrow(df2)
df2$Date <- lubridate::as_date(df2$Date)
...
Jetzt haben Sie ein Paket installiert, nämlich `cowsay`, was wir in diesem Tutorial nutzen möchten. Nun liegt es mit allen anderen installierten Paketen in einer "Bibliothek" auf Ihrem Rechner. Um es aber tatsächlich nutzen zu können, fehlt noch ein letzter Schritt, es muss aus der Bibliothek geholt werden und in die aktive Sitzung geladen werden.

Es gibt zwei Wege, an den Inhalt eines bereits installierten Pakets zu gelangen:

1. Sie laden das gesamte Paket in die aktive Sitzung mit `library(Paketname)`.
1. Sie addressiern nur bestimmte Inhalte aus dem Paket mit `Paketname::Inhalt`.

Beide Varianten werden Ihnen begegnen. Wobei die erste (`library()`) häufig
...
::: aufgabe
Laden Sie das gesamte Paket `cowsay` in die aktive Sitzung, um es im nächsten Schritt zu benutzen!
:::
```{r lib, exercise = TRUE}
