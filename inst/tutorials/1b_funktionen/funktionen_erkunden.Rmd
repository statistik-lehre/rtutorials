...
::: blau-nb
Um in RStudio die Hilfe aufzurufen, gehen Sie mit dem Cursor in den Namen der Funktion (z.B. `say`) und drücken F1. Das funktioniert aber nicht im interaktiven Codeblock.
\
...
::: aufgabe
Nutzen Sie `?`, um die Hilfeseite für die Funktion`say()` aufzurufen!
Schauen Sie selektiv nach der Information, welche Tiere alle beim Argument `by` möglich sind. 

Blenden Sie Unverständliches und schlechte Formatierung möglichst aus (weil wir uns im Tutorial befinden werden die Überschriften komisch dargestellt).
:::

<br>

```{r help_say, exercise = TRUE, exercise.setup = "say", exercise.cap = "Rufen Sie die Hilfe auf!" }
?say

```


```{r rms, exercise = TRUE, exercise.setup = "say", exercise.cap = "Probieren Sie andere Tiere aus!" }
...
Diese Default-Werte finden Sie in den Hilfeseiten unter dem Abschnitt **Usage**, und sie werden Ihnen in RStudio angezeigt in einem kleinen gelben Kästchen, sobald Sie die Klammer hinter dem Funktionsnamen tippen.
...
Das erleichtert anderen, die den Code mal lesen müssen (z.B. Ihrem zukünftigen Ich), schneller zu erfassen worum es geht. 
...
Falls Sie noch kein Projekt erstellt haben, sollten Sie das tun - gehen Sie in dafür zurück zum letzten Tutorial, wo das erklärt wird.

::: aufgaberstudio
**Aufgabe 1**

Speichern Sie Ihr Skript einfach in den Projektordner! Nutzen Sie dafür `Strg + S` oder das grafische Menü unter "File" und dann auf "Save" klicken.

**Aufgabe 2**

Schreiben Sie im Skript eine kleine Geschichte mithilfe des Pakets `cowsay`.\
Lassen Sie verschiedene Protagonisten sprechen. Der Umfang liegt in Ihrem Ermessen.\
Speichern Sie alles.\
Achten Sie dabei auf Lesbarkeit und korrekte Syntax.\

(Sie dürfen sich gerne auch von bereits existierenden Geschichten inspirieren lassen oder diese nachstellen.)
:::
...
Erstellen Sie ein neues R Notebook über ![](images/skript.PNG) und dann "R Notebook" auswählen. Speichern Sie es in Ihrem Projekt.
...
Es erscheint ein neues Dokument, was ein paar Beispiele enthält. 

::: aufgaberstudio
**Aufgabe 2**

Speichern Sie das Dokument zunächst in Ihren Projektordner.

Nutzen Sie dann den ![](images/preview.png)-Knopf, um sich eine Vorschau des fertigen Dokuments erstellen zu lassen.

Vergleichen Sie die „Rohform“ mit der „gerenderten“ Form. 
...
Um zu verstehen, was genau Sie sehen, sollten Sie zunächst etwas über `Markdown` lernen.
...
Um sich im R Notebook einfach zurechtzufinden, können Sie den [Visual Editor](https://rstudio.github.io/visual-markdown-editing/) einschalten. Das geht mit `Strg + Shift + F4` oder der Schaltfläche unten links in der Leiste des Editors.
...
Wenn Sie jemandem ein Skript geben, wo nur der Code enthalten ist, müsste die Person erst mal R starten, das Skript öffnen, die nötigen Pakete installieren, und das Skript ausführen, um irgendwelche Ergebnisse zu sehen. Natürlich macht das niemand. 

Die andere Option, nur den Output ohne Quellcode weiterzugeben, wird oft praktiziert - Sie erstellen eine Grafik mit R, speichern sie, und geben Sie an eine Kollegin weiter. Diese möchte aber vielleicht verstehen wie diese Grafik entstanden ist, oder einen Fehler korrigieren, kann die Grafik aber nicht reproduzieren, weil sie nicht Ihren Code hat sondern nur den Output.

Der Königsweg ist also, Code und Output zusammen übersichtlich darzustellen.
Wenn Sie jetzt also eine Geschichte schreiben, möchte ich Sie ermuntern, das in einem R Notebook zu speichern. So können andere viel schneller sehen, welche schönen Tiere Sie sich ausgesucht haben, und es ist viel aussagekräftiger als ein bloßes Skript.

Ein R Notebook ist, als würden Sie Drehbuch und Film nebeneinanderlegen. 

::: aufgaberstudio
**Aufgabe 3**

1. Kopieren Sie Ihre Geschichte aus dem Skript in das R Notebook! Fügen Sie den Code innerhalb des Codeblocks ein.

2. Führen Sie den Codeblock aus.

3. Lassen Sie sich ein ![](images/preview.png) anzeigen.

4. Passen Sie den "normalen" Text außerhalb des Codeblocks an Ihre Geschichte an.

5. Speichern Sie das R Notebook mit `Strg + S` oder über das Menü.
:::
...
Sie können einen Codeblock erstellen mit `Ctrl + Shift + i`. Falls Sie sich im Visual Mode befinden, geht es auch über das "Insert"-Menü. 
...
```{r quiz}
learnr::quiz(caption = "Testen Sie Ihr Verständnis",
             ...
             learnr::question_radio("Wofür steht das ‚ML‘ in „HTML“?",
                                    ...
                                    allow_retry = TRUE,
                                    random_answer_order = TRUE),
             learnr::question_text("Welche Funktion verwenden Sie, um den gesamten Inhalt eines Pakets in die aktive Sitzung zu laden?",
                                   answer("library()", correct = TRUE),
                                   answer("library", correct = TRUE),
                                   allow_retry = TRUE
                                   ),
             learnr::question_radio("Braucht es Anführungszeichen um den Paketnamen, wenn Sie ein Paket mit `install.packages()` installieren möchten?",
                                   answer("Ja", correct = TRUE),
                                   allow_retry = TRUE,
                                   random_answer_order = TRUE,
                                   answer("Nein")),
             learnr::question_radio("Wenn Sie ein Skript schreiben, wo im Skript laden Sie alle benötigten Pakete?",
                                    ...
                                    allow_retry = TRUE,
                                    random_answer_order = TRUE),
             question_radio("Wie markieren Sie für R, dass es sich bei Ihrer Eingabe um Text handelt?",
             ...
             question_checkbox("Wählen Sie alle möglichen Wege aus, die Hilfe zur Funktion `say()` aufzurufen",
                            answer("`?say`", correct = TRUE),
                            answer("`help(say)`", correct = TRUE),
                            answer("Den Cursor in die Funktion bewegen und F1 drücken", correct = TRUE, message = "Das mit F1 funktioniert nicht innerhalb der interaktiven Tutorials, aber wenn Sie in RStudio etwas im Skript oder der Konsole schreiben."),
                            allow_retry = TRUE,
                            random_answer_order = TRUE),
             ...
             question_radio("Was ist der Standardprozess, um ein Paket von CRAN zu installieren?",
                           answer("Die Webseite von CRAN aufrufen und von dort das Paket herunterladen, entpacken, dann an den Ort Ihrer Bibliothek legen und installieren, in dem Sie den Befehl `install.packages()` verwenden.", 
                                  message = "Das ist zwar eine mögliche Art, aber sehr umständlich und eigentlich auch nur nötig, wenn Sie keine Internetverbindung haben. Ohne Internet ist es praktisch, Pakete an einem Gerät mit Internet herunterzuladen, dann über USB oder ähnliches auf das Gerät ohne Internet schieben zu können und dann installieren. Das ist aber eher selten und nicht die Standardprozedur."),
...
Heute haben Sie gelernt, dass das R Ökosystem unter anderem aus 20 000 Paketen besteht, die von Mitgliedern der Community entwickelt und gepflegt werden. 
...
Wenn Sie einem Argument Text übergeben wollen, muss dieser von `"` umschlossen sein, um auch als solcher erkannt zu werden. Alles, was nicht so gekennzeichnet ist, wird von R als Aufruf eines bereits vorhandenen Objekts interpretiert. Beispielsweise als eine Funktion oder eine Variable.\
