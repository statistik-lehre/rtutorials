...
Was R so mächtig macht, sind die von der Community veröffentlichten und gepflegten Erweiterungen drum herum. Davon gibt es sehr viele, die aktuelle Zahl (über 20.000) sieht du unten in der Grafik. Sie zeigt das Veröffentlichungsdatum aller aktuellen Pakte auf CRAN. Was ist CRAN?

Dieses Akronym steht für *Comprehensive R Archive Network* und ist die offizielle Adresse für geprüfte Erweiterungen. (Es sei aber erwähnt, dass es auch noch andere Orte gibt, wo Pakete veröffentlicht werden können, zum Beispiel Bioconductor, GitHub und andere.) Der Vorteil dieser zentralen Instanz **CRAN** ist, dass Regeln durchgesetzt werden und damit die Pakete untereinander alle erstaunlich kompatibel und konfliktfrei funktionieren. Wer andere Programmiersprachen kennt (wie z.B. Python) weiß, dass das überhaupt nicht selbstverständlich ist. Das ist mit R sehr komfortabel!

Pakete von CRAN können über den einfachen Befehl `install.packages("Paketname")` installiert werden - das ist unkompliziert, du musst keine Internetseiten besuchen, keine Dateien irgendwo hinschieben oder ausführen, das wird alles intern von dieser Funktion erledigt.
...
::: infobox
Übrigens, Pakete muss man nur einmal zu Beginn installieren und dann hat man sie! Deswegen lohnt es sich den Code direkt in die Konsole zu tippen und gar nicht erst ins Skript aufzunehmen. 
:::
...
craneval <- function(x){
  s <- tolower(x)
  ifelse(s == "comprehensive r archive network",
         return(correct()),
         return(incorrect()))
  }
